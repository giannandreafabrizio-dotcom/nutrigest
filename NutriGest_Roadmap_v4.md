...
**SCHEDA:** Stato: **Fase 0.5 ✅ CHIUSA e collaudata (16 lug 2026, commit `6ec732e`)** — variante **B2 pull differenziale**, dettagli nel CHANGELOG. **Fase 1 IN CORSO, sotto-passi 1a+1b fatti (16 lug 2026, notte):** 1a — tabella `collections` creata su Supabase (`{key,user_id,data jsonb,updated_at}`, RLS row-owner identica alle altre 5 tabelle, verificata da Fabrizio); 1b — doppia scrittura attiva (`_collectionsUpsert`) su tutti i 5 punti che scrivono i 4 meta-record, gated sul successo della scrittura legacy, **nessun cambio di lettura** (comportamento app invariato). Prossimo passo: lasciar popolare `collections` con l'uso reale per qualche giorno, verifica manuale nel Table Editor, poi 1c (doppia lettura con preferenza al nuovo) · Priorità: Media-Alta · C: 5 | I: 5 | R: 4 · Modello: Fable 5 / Opus / Sonnet (sessione con più cambi modello, vedi CHANGELOG) · Autonomia: **L0**.
...
**SEGUITO PIANIFICATO — pannello app (Fabrizio, ~fine luglio/inizio agosto 2026, "tra ~2 settimane"):** versione app del tracciamento — un pannello sotto al piano dove Fabrizio (per ora) registra settimana per settimana la scelta del paziente + alcol, salvato nella scheda e consultabile come storico. È la **versione leggera di P85 (diario paziente)**: quando esisterà l'app paziente (P50) sarà il paziente stesso a confermare pasto per pasto (rispettato / più / meno) e a segnare il sabato, generando il proprio report kcal settimanale. Tocca i dati salvati → si farà a tappe con approvazione (L1). Nota rispetto alla critica CTO originale ("il tracking appartiene a P85, non al piano"): resta valida — il PDF fa solo auto-monitoraggio su carta; il dato salvato vivrà nel pannello/diario, non nella struttura del piano.
