...
### P157 — La sync spinge tutto PRIMA di idratare: il difetto che ha sovrascritto sei pazienti 🔴 DA FARE PRIMA DI OGNI LAVORO MULTI-DISPOSITIVO (aperta notte 11-12 ago 2026)
**IL FATTO (visto in diretta, log console 23:09):** `syncNow` fa `pull lista` -> **push di TUTTI i 41 pazienti coi blob in memoria** -> idratazione dei cambiati. L'ordine e rovesciato: chi ha memoria vecchia riscrive il server un istante prima di scaricare il nuovo. Cosi sono stati persi i referti di sei pazienti (recuperati dal backup del 9 ago — verbale nel CHANGELOG dell'11-12 ago), e cosi il primo ripristino e durato tre minuti.
**SECONDA GAMBA:** la baseline P69 avanza anche senza idratazione del blob — 5 pazienti su 6 non si idratavano ne al boot ne su `openPaz`, la baseline li dichiarava freschi mentre il locale era di meta luglio. Da trovare dove la baseline viene scritta senza che `db.pazienti[i]` sia sostituito.
**LA CORREZIONE (da progettare a mente fresca, non di notte):** (1) idratare PRIMA di spingere; (2) push solo degli sporchi veri, non di tutti; (3) guardia per riga: non sovrascrivere una riga il cui `updated_at` remoto e piu nuovo della baseline locale — e il conflitto P69, va MOSTRATO, non vinto in silenzio; (4) un test che simuli il dispositivo rimasto indietro.
**FINCHE NON E CHIUSA:** una sola scheda, un solo dispositivo, l'altro PC non apre l'app. La tabella `_pre_ripristino_20260811` si elimina solo dopo il collaudo di questa voce.
**SCHEDA:** Stato: 🔴 Da fare SUBITO · Priorita: MASSIMA (integrita dati clinici) · Categoria: Sync / Supabase · Dipendenze: nessuna · Complessita 3 | Impatto 5 | Rischio 5 · Autonomia LLM: L1 — progetto e test insieme a Fabrizio, nessun push senza collaudo
