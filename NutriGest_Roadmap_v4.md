...
- **Tappa 2 — Da fare:** portare sullo stesso motore/registro gli invii oggi non tracciati (PDF piano `apriWhatsApp`, messaggi AI `apriAIWhatsApp`) e migrare `p.richiesteAnalisi[]` → `p.invii[]`.
- **Tappa 3 — Da fare:** tab "Comunicazione" = vista su `p.invii[]` + template variabilizzati ({nome},{link},{prossimo appuntamento}) + bottoni wa.me/mailto. Business API: si rivaluta con P50, non prima.
**SCHEDA:** Stato: **In corso (Tappa 1 chiusa)** · Priorità: Bassa · C: 2 | I: 3 | R: 1 · Modello: Sonnet (Media) · Autonomia: L1.
