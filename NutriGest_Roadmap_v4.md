...
**Come si legge questo file (regola dal 18 lug 2026):** lo stato ufficiale di ogni voce vive **SOLO nella sua SCHEDA** qui sotto (Stato · Priorità · dipendenze · note di chiusura). La cronologia completa — cosa è successo, quando, con quale commit — vive **SOLO nel CHANGELOG.md**. Questo file non contiene più riepiloghi di stato duplicati: se una frase fuori dalle schede contraddice una scheda, **fa fede la scheda**. Il vecchio riepilogo che stava qui è stato spostato integralmente nel CHANGELOG (voce del 18 lug 2026).

**Ultimo allineamento — PARZIALE: 4 agosto 2026, codice al commit `c9fffac`.** Attenzione a cosa vuol dire «parziale»: in quella passata sono state riverificate riga per riga contro `index.html` e `CHANGELOG.md` **soltanto** le voci **P61, P4, P37, P122, P124b, P73, P66c, P40, P35, P65** e la sigla **F5** (ogni correzione è marcata «verificato 4 ago 2026» nella scheda relativa). **Tutto il resto del file NON è stato riverificato al commit `c9fffac`**: per le altre voci l'ultimo allineamento generale resta quello del **18 lug 2026, commit `e536b95`**. Questa riga non è una garanzia sull'intero documento.
...
> 🚫 **QUESTA TABELLA NON È UNA FONTE DI STATO. (verificato 4 ago 2026)** Serve a **una cosa sola**: dire *quale modello e quale effort* usare quando si lavora su una voce. Se una riga qui dice o lascia intendere che una voce è aperta, chiusa, da fare o bloccata, **quell'informazione non conta**: lo stato ufficiale vive **solo nella SCHEDA della voce**, più in basso in questo file. La presenza di una voce in questa tabella **non significa che sia da fare**.
> **Perché sta scritto qui:** l'audit del 4 agosto 2026 ha trovato che è proprio da questa tabella che sono nate le due divergenze più gravi del file — **P73** (elencata qui come lavoro da fare, Opus/High/ON, mentre la sua scheda la dà CHIUSA dal 16 lug 2026, commit `34dd1ae`) e **P61** (che la tabella non barrava e la cui scheda diceva ancora «Da fare», mentre è chiusa dal 7 lug 2026, commit `ed1e3e9`). Entrambe corrette il 4 ago 2026. Regola operativa: **quando una voce si chiude, la sua riga qui si barra** (`~~P__~~ ✅`), come già fatto per P68, P105, P106, P108, P109, P111, P112, P55, P119-P122, F5-salvaPaz, F6, F7, F9.

| Voce / blocco | Modello | Effort | Thinking | Perché |
|---|---|---|---|---|
| **P63b** controlli coerenza import InBody | Opus | Alto | ON | ✅ Chiusa 31 lug 2026 in forma ridotta (no tabella-spunte, sì controlli aritmetici) — da collaudare |
| **P65** repo privato | — (manuale) | — | — | Operazione GitHub di Fabrizio, 5 minuti |
| ~~**P66c**~~ ✅ chiusa 18 lug 2026 (chiusura trasporto diretto legacy, commit `e536b95` — v. scheda) | — | — | — | — |
...
| ~~**P73**~~ ✅ chiusa 16 lug 2026 (revisione linguaggio prescrittivo, commit `34dd1ae` — v. scheda; resta solo il punto 4, lista sentinella come test, spostato dentro P78) | — | — | — | — |
...
| ~~**F5-salvaPaz**~~ ✅ chiusa 25 lug 2026 (l'anagrafica cancellava percorso/referti/richieste — v. scheda P122) | Opus | High | ON | Perdita silenziosa di dati clinici |
...
> ⚠️ **COLLISIONE DI SIGLA — «F5» ha significato due cose diverse. (verificato 4 ago 2026)** Fino al 4 agosto 2026 la sigla `F5` indicava **due findings distinti**, entrambi veri: **(1) F5 — Vittoria gratuita** (questa qui sopra: proiezioni PostgREST sulla lista pazienti, poi diventata fase 0.5 di P74, dei findings trasversali dell'8 lug 2026); **(2) F5** = il bug del 25 lug 2026 in cui `salvaPaz` cancellava in silenzio percorso, referti del sangue e richieste esami. Per togliere l'ambiguità il **secondo** è stato rinominato **`F5-salvaPaz`** in tutto questo file; `F5` senza suffisso resta la vittoria gratuita PostgREST. Nel **CHANGELOG.md** la serie `F5/F6/F7` (voci del 25-26 lug 2026) si riferisce sempre a **`F5-salvaPaz`**, e il CHANGELOG non è stato rinominato: è storia già scritta.

---

# PRIORITÀ 0 — Sicurezza clinica e del dato

### P63 — Conferma con diff per gli import AI dei referti ✅ CHIUSA (commit `8c9e77a`, 8 lug 2026 — Analisi del sangue; l'InBody è coperto in forma diversa da P63b, 31 lug 2026)
...
### P124 — Import referti del sangue a prova di errore di lettura ✅ CHIUSA E COLLAUDATA 26 luglio 2026 (collaudo in due giri: il primo ha generato P124b, il secondo superato)
**DA DOVE NASCE:** caso reale del 26/7. Su un referto fotografato (una pagina ruotata di 90°), su 16 valori estratti **7 erano sbagliati**: cinque erano l'intervallo di riferimento copiato al posto del risultato (Creatinina `0.72-1.18`, e-GFR `89-98`, Azotemia `30-25`, B12 `197-771`, Folati `4.5-23.2`), uno aveva perso la virgola (TSH `1560` invece di `1,560`) e uno una cifra (Vitamina D `2.3` invece di `21,3`). Fabrizio se n'è accorto guardando, e **non è potuto intervenire**: la finestra di conferma di P63 era di sola lettura.
**IL PROBLEMA DI FONDO:** l'estrazione AI veniva trattata come un dato e non come una proposta. Gli unici avvisi esistenti (fuori range, delta >50%) giudicavano la *clinica* del valore, non la *plausibilità della lettura* — e tacevano proprio sui valori inventati, perché `0.72-1.18` non è un numero e `1560` non ha un precedente con cui confrontarsi.
**LA SOLUZIONE (tre pezzi, tutti nella stessa consegna):**
**A — correzione a mano nella finestra di conferma.** La colonna "Estratto" è un campo scrivibile, col riferimento dell'esame stampato accanto al nome. Correggere una riga sospetta **rimette la spunta da sola**. Il valore corretto a mano è tracciato come `ai-import-corretto` in `p._analisiMeta` (distinto da `ai-import`): misura nel tempo quanto e dove l'AI sbaglia.
**B — tre controlli deterministici** (`_impControllaValore`), indipendenti dall'AI e quindi funzionanti *proprio quando* l'AI sbaglia: forma di intervallo · stesse cifre del range stampato dal laboratorio · ordine di grandezza oltre il fattore 10 rispetto a `RANGE_STD`. Righe sospette **deselezionate**, in rosso, con banner esplicativo.
**C — prompt riscritto:** copiare il numero cifra per cifra come stampato (virgola italiana compresa; la conversione la fa `_impNormalizzaNumero`), istruzione esplicita colonna Esito/Risultato vs colonna dei riferimenti, e **omettere** l'esame quando la lettura è incerta.
**LA DECISIONE CHIAVE — quando NON suggerire:** la correzione automatica ("usa 1.56") si propone SOLO se lo spostamento della virgola riporta il valore dentro il riferimento. Sulla Vitamina D, 2.3 × 10 = 23 sarebbe stato plausibile e sbagliato (il vero era 21.3): lì l'app dice solo "ricontrollalo sul referto". *Un suggerimento sbagliato è peggio di nessun suggerimento: il primo si accetta, il secondo si controlla.*
**SCARTATO PER ORA (opzione D):** pulsante "rileggi questo valore" con una seconda chiamata AI mirata sul singolo esame. Costa una chiamata a click e i tre controlli prendono già 7 errori su 7 del caso reale. Si riapre se il collaudo mostra errori che sfuggono a tutti e tre — tipicamente un numero plausibile e dentro il riferimento (Trigliceridi 91 letto 21): contro quello esiste solo l'occhio umano, ed è il motivo per cui A viene prima di B.
**COLLAUDO DA FARE:** ricaricare il referto del caso reale e verificare **(1)** che le cinque righe-intervallo arrivino in rosso e deselezionate; **(2)** che il TSH proponga "usa 1.56" e che il click funzioni; **(3)** che la Vitamina D sia segnalata **senza** proporre 23; **(4)** che correggendo a mano `2.3` → `21,3` la spunta si rimetta e il valore salvato sia `21.3`; **(5)** che i valori corretti (Ferritina, Ferro, FT3, FT4…) NON siano segnalati — un allarme che suona sempre è un allarme che si impara a ignorare; **(6)** che il referto creato contenga solo le righe spuntate.
**SECONDO COLLAUDO (26/7, stesso giorno) → P124b.** Ricaricato il referto, stavolta come PDF a 4 pagine: **~25 valori sbagliati su 32**, ma in modo NUOVO — riga sbagliata, non colonna. Piastrine prendeva il valore dell'Emoglobina, TSH quello di FT4; come "riferimento" tornavano `pg` e `migliaia/mmc`. **Causa:** il PDF è una scansione senza testo con le pagine ruotate di 90°, e **la rotazione dichiarata nel file era sbagliata** (`/Rotate 270`, ma l'orientamento giusto è 180 — provate tutte e quattro in un browser vero). Correzione in tre pezzi: **(1)** le pagine le rende l'app con pdf.js, dritte, con la rotazione **rilevata** da una chiamata piccola (vale anche per le foto storte); **(2)** una pagina per chiamata; **(3)** l'**impronta della riga** — voce, valore, unità e riferimento chiesti dalla stessa riga e controllati fra loro: se il riferimento non è un intervallo o l'unità non è quella dell'esame, la riga è disallineata e viene scartata (14 righe sbagliate su 25 prese da questi due soli controlli). Verificato in Chromium headless sul PDF vero. Test 318 → **327**. Nuove dipendenze: `vendor/pdf.min.js`, `vendor/pdf.worker.min.js`.
**COLLAUDO P124b: SUPERATO (26/7), con tre code corrette nella stessa giornata.** La terza: i PREFISSI delle unità (nanomoli/micromoli/millimoli) facevano scattare l'allarme «riga disallineata» su righe giuste — Omocisteina 11,3 e Creatinina 97 µmol/L. `_impStessaGrandezza` ora confronta la grandezza fisica a meno del prefisso; il caso in cui la scala conta davvero resta coperto dall'ordine di grandezza. Test 330 → **334**.
**COLLAUDO P124b: SUPERATO (26/7).** Ricaricato il PDF vero: pagine raddrizzate di 270°, emocromo corretto (GB 5,0 · GR 5,2 · Neutrofili 2,7 · Linfociti 1,7), TSH e FT4 al posto giusto. Niente più scivolamento di riga. Due code corrette subito: **(a)** i leucociti compaiono in percentuale E in valore assoluto e venivano segnalati come conflitto su cinque voci — ora fra due righe dello stesso esame si tiene quella con l'unità attesa, e il conflitto resta solo quando l'unità non decide; **(b)** come data del referto era stata letta la **data di nascita** del paziente (06/03/1990 invece del 17/06/2026): data plausibile e formalmente corretta, quindi invisibile — ora il prompt avverte e l'app rifiuta la data se coincide con `p.nascita` o è nel futuro. Test 327 → **334**.
> 📓 **STORICO — NON È LAVORO DA FARE. (verificato 4 ago 2026)** La lista qui sotto è la **checklist originale** scritta *prima* del collaudo di P124b, conservata per memoria di cosa era stato chiesto di verificare. **Quel collaudo è stato eseguito e SUPERATO il 26/7** — lo dicono le due righe «COLLAUDO P124b: SUPERATO (26/7)» immediatamente sopra, che riportano gli esiti punto per punto (pagine raddrizzate di 270°, emocromo corretto, TSH e FT4 al posto giusto, righe disallineate deselezionate col motivo). **Non rieseguire questa lista credendo che sia aperta**: la dicitura «DA FARE» qui sotto è il testo d'epoca, non lo stato attuale. Lo stato attuale della voce è nella SCHEDA in fondo. *(Titolo storico conservato integralmente sotto.)*
**~~COLLAUDO P124b DA FARE~~ → SUPERATO IL 26/7/2026** *(testo originale, pre-collaudo)*: ricaricare il PDF di Mangini e controllare **(1)** che compaia l'avviso "↻ Pagine raddrizzate di N°"; **(2)** che i valori dell'emocromo corrispondano davvero al referto (Globuli bianchi 5,0 · Emoglobina 15,1 · Piastrine 173 · MCV 85,7); **(3)** che TSH sia 1,560 e FT4 1,17, non scambiati; **(4)** che le righe eventualmente disallineate arrivino deselezionate col motivo scritto.
**SCHEDA:** Stato: ✅ **CHIUSA E COLLAUDATA il 26 lug 2026** *(riga corretta il 10 ago 2026: diceva «CHIUSA, da collaudare» mentre quattro righe più sopra lo stesso blocco scrive due volte «COLLAUDO P124b: SUPERATO (26/7)». Il collaudo è avvenuto in due giri lo stesso giorno: il primo ha trovato ~25 valori sbagliati su 32 in forma nuova — riga sbagliata invece di colonna — e ha generato **P124b**; il secondo, sul PDF vero, è **superato** con tre code corrette in giornata. La correzione del 4 agosto aveva sistemato il corpo della voce e lasciato la scheda, cioè proprio il punto a cui la correzione stessa rimandava.)* · Priorità: Alta · Categoria: Sicurezza clinica / Import AI · Modello: Opus · Autonomia: L1 · Test 305 → **330** (`s2-import-referto-controlli.test.js`), in due tappe: P124 (correzione a mano + controlli) e P124b (pagine dritte + impronta della riga). Estende P63 (staging + diff) e P118 (referti datati, range del laboratorio), che restano invariate.

### P125 — Ricerca fra i parametri + LDL stimato (Friedewald) ✅ CHIUSA 26 luglio 2026 — **DA COLLAUDARE**
**DA DOVE NASCE:** tre proposte di Fabrizio a fine giornata, discusse prima di scrivere codice. Due fatte, una rimandata di proposito.
**1. RICERCA.** 119 voci in 23 sezioni: trovarne una voleva dire aprire le sezioni a una a una. Casella in cima alla scheda, filtro dal vivo, sezioni con risultati aperte da sole e richiuse alla cancellazione, ESC pulisce, ricerca anche sui titoli di sezione. **Il punto tecnico che decide tutto:** corrispondenza per **parole-prefisso** e non per sottostringa — `"vit d"` come sottostringa NON esiste dentro "vitamina d". Un test verifica che tutte e 119 le voci siano raggiungibili dalla loro prima parola.
**2. LDL STIMATO.** `Totale − HDL − TG/5`, nel pannello dei calcoli derivati. **Non scrive nella casella LDL**: un valore calcolato in un campo di valori misurati è una doppia fonte, la famiglia F4/P118/P120. **Nessun semaforo**, coerente con RANGE_RIF livello B: il target LDL dipende dal rischio CV del singolo. **Campo di validità applicato:** sopra 400 di trigliceridi non calcola e dice perché (inapplicabile ≠ impreciso); avvisa se TG>200 (sottostima → non-HDL), se la stima è sotto 70, e se il laboratorio ha misurato un LDL diverso (fa fede il misurato). Agganci `valido`/`avviso`/`dec` aggiunti a `calcolaIndice` in forma **generica**, non come caso speciale dell'LDL.
**RIMANDATA — elettroforesi proteica in frazioni.** Oggi è una sola casella di testo libero: non entra in nessun calcolo, non ha andamento, non ha semaforo. Trasformarla in 5 frazioni (albumina, α1, α2, β, γ) richiede decisioni cliniche. **Decisione già presa: percentuali** (sempre stampate, riferimenti standard) — scelta fatta apposta per non ripetere il pasticcio % / valore assoluto dei leucociti di P124b. Da valutare l'avviso sul picco monoclonale nelle γ: se ci sarà dovrà dire "manda dal medico", non interpretare.
**COLLAUDO DA FARE:** **(1)** scrivere `vit d`, `ferrit`, `glob bianc`, `tiroid` e controllare che trovi quello che ti aspetti; **(2)** cancellare la ricerca → le sezioni tornano come prima (chiuse se vuote); **(3)** su un paziente con colesterolo, HDL e trigliceridi compilati, la scheda **LDL stimato (Friedewald)** nel pannello calcoli; **(4)** che la casella LDL resti **vuota**; **(5)** su un paziente con LDL misurato dal laboratorio, la nota di confronto sotto il numero.
**SCHEDA:** Stato: **CHIUSA, da collaudare** · Priorità: Media · Categoria: Usabilità + calcoli clinici · Modello: Opus · Autonomia: L1 · Test 334 → **350** (`s2-analisi-ricerca-ldl.test.js`).

### P63b — Controlli di coerenza sull'import InBody ✅ CHIUSA in forma ridotta 31 lug 2026 — **DA COLLAUDARE**
...
**RIAPERTA E CHIUSA IN FORMA DIVERSA — 31 luglio 2026.** La decisione dell'8 luglio poggiava su un presupposto mai verificato: che gli errori di lettura fossero rari. Il 31 luglio Fabrizio ha caricato **25 referti veri**: circa 1 su 2 con almeno un errore, 1 su 5 con un errore grave (6-7 sul grasso viscerale, 4-5 numeri confusi — il peggiore una **massa magra di 45 kg letta 88 kg**, plausibile a colpo d'occhio e quindi invisibile).
**NON è tornata la tabella con le spunte** — quel no resta valido, costerebbe un clic anche quando è tutto giusto. È entrato invece il **pezzo B di P124** tradotto sull'InBody: controlli deterministici che parlano solo quando i conti non tornano. E qui sono più forti che sul sangue, perché **i valori InBody sono legati da identità aritmetiche** ("peso = massa grassa + massa magra" è la definizione con cui la bilancia stampa il foglio): l'errore si trova con una sottrazione, senza AI e senza il referto.
`_ibControllaCoerenza` (pura) — somma · % grassa · BMI · acqua/massa magra · muscolare ≤ magra · viscerale 1-20. Più l'avviso fisso sotto il pulsante di import, che **nomina i tre campi che hanno davvero sbagliato** (dire "ricontrolla i numeri" non dice all'occhio dove guardare). Tolleranze larghe di proposito; non blocca il salvataggio (a differenza della data di P120).
**PERCHÉ IL CROSS-CHECK MIFFLIN NON BASTAVA:** `calcolaTDEE` usa `lastIb.mb`, il MB **letto dal referto**, non ricavato dalla massa magra — quindi una massa magra sbagliata non sposta le calorie, sposta `_avvisoProteineDeficit` (soglia su g/kg FFM), la linea massa magra del Percorso e la riga del contesto AI. Il danno è nel **giudizio clinico**, non nel numero.
**RESTA FUORI, DICHIARATO:** la scansione retroattiva dei referti già in archivio (Fabrizio ha corretto i 25 a mano). **Non richiede i PDF** — legge i numeri già in cartella — quindi resta disponibile a costo basso se in futuro si vuole sapere quanti dati storti sono ancora in giro. Il parser del grasso viscerale resta **P100**: è un valore isolato, nessuna somma lo smaschera; ora però i referti veri che falliscono esistono.
**SCHEDA:** Stato: ✅ **CHIUSA in forma ridotta** (31 lug 2026) — **DA COLLAUDARE** · Priorità: ALTA · Categoria: Sicurezza clinica / InBody · Dipendenze: nessuna · Autonomia: L1 · Test 452 → **472** (`s2-inbody-coerenza.test.js`).
**COLLAUDO DA FARE:** **(1)** caricare un referto corretto e verificare che **non compaia nulla** (è la prova più importante: un avviso che suona sempre smette di essere letto); **(2)** mettere a mano una massa magra doppia e controllare che compaia il riquadro rosso e che i tre campi si bordino; **(3)** correggere il valore e verificare che l'allarme sparisca **da solo**, senza premere nulla; **(4)** chiudere e riaprire il modale: nessun allarme ereditato; **(5)** che il salvataggio resti possibile anche con l'allarme acceso.
...
**LA SOLUZIONE OTTIMIZZATA:** decidere ORA in 10 minuti: se P66 si farà entro 2-3 mesi → migrare subito a Vercel (repo privato gratis, hosting gratis, Functions pronte per P66, preview deploy) e chiudere P51; altrimenti pagare GitHub e rimandare. ~~Scan dello storico commit per segreti in ogni caso (l'anon key è pubblica by design, ma verificare che non sia MAI passata una service_role o la chiave Anthropic).~~ → **✅ SCAN GIÀ FATTO, non ripeterlo (v. sotto).**
**✅ LO SCAN DELLO STORICO È STATO FATTO — 13 luglio 2026 (verificato 4 ago 2026).** Prova: `grep -n "460" CHANGELOG.md` → sessione pomeridiana/serale del 13 lug 2026, *«P65 — scan storico Git (**460 commit**) per segreti prima di rendere il repo privato: **nessuna chiave reale trovata** (solo placeholder UI `sk-ant-...`), **nessuna service_role/AWS key**, **nessun `.env` committato**, il file locale `password api keys.txt` non è mai stato tracciato. Repo pronto per essere reso privato; resta da decidere GitHub a pagamento vs migrazione a Vercel (P51)»*. Quindi il prerequisito di sicurezza è **soddisfatto** e il repo è **già pronto** a diventare privato: rifare lo scan sarebbe lavoro a vuoto (andrebbe semmai solo riesteso ai commit successivi al 13 lug, se si vuole essere pignoli).
**QUELLO CHE RESTA APERTO È SOLO UNA DECISIONE, non un lavoro tecnico:** **GitHub a pagamento** (per avere Pages su repo privato) **vs migrazione a Vercel** (repo privato + hosting gratis, e chiude P51). Nessun codice da scrivere in nessuno dei due casi. Nota di contesto: P66 Fase 0+1 è chiusa dall'8 lug 2026, quindi la condizione «se P66 si farà entro 2-3 mesi» che doveva guidare la scelta si è già **avverata** — l'ipotesi Vercel è quella che il ragionamento originale indicava.
**FOCUS COMPONENTI COINVOLTI:** Infrastruttura. Zero codice (o un file di config Vercel).
**SCHEDA:** Stato: **Aperta solo per la decisione GitHub-a-pagamento vs Vercel** — scan dello storico (460 commit) **già eseguito il 13 lug 2026, nessun segreto trovato**; repo pronto a essere reso privato · Priorità: Alta (costo ~zero) · C: 1 | I: 3 | R: 1 · Modello: nessuno (operazione manuale guidata) · Autonomia: L0 (esegue Fabrizio).
...
2. ✅ **Modificatore lavoro nel NEAT** (22 lug 2026, pushato commit `f263ad1`, collaudato in produzione da Fabrizio lo stesso giorno): campi `fontePassi` (Misurati/Stimati) e `tipoLavoro` (Sedentario/In piedi/Pesante); bonus frazione MB (in-piedi +0.06, pesante +0.13) sommato alla frazione NEAT, con tetto totale 0.60. Anti doppio conteggio: bonus applicato SOLO quando `fontePassi ≠ 'misurati'` (con passi da smartwatch lo stare in piedi è già contato). Un lavoro attivo basta ad attivare il metodo MET anche senza passi. Test dedicati (`s2-tdee-lavoro-affidabilita.test.js`), suite 68→77.
...
5. ✅ **Indice di affidabilità della stima** (22 lug 2026, pushato commit `f263ad1`, collaudato in produzione da Fabrizio lo stesso giorno): `_affidabilitaTDEE` classifica 🟢 alta / 🟡 media / 🔴 bassa a punti sui dati disponibili (passi misurati vs stimati vs mancanti · allenamento in minuti vs ore approssimate · InBody >4 mesi · LAF manuale) e ne ricava un intervallo (±8/13/18%, ±20% su LAF manuale). Reso via `_affidabilitaHtml` nei box TDEE (apertura, Ricalcola, salvataggio). Non entra in nessun calcolo. Il passo 7 (cross-check Mifflin) si aggancia qui.
...
7. ✅ **Cross-check Mifflin-St Jeor come guardia** (24 lug 2026) — NON formula alternativa: `_mifflinBMR`/`_crossCheckMifflin` dentro `calcolaTDEE` calcolano il MB teorico (10×kg + 6.25×cm − 5×anni + M:+5/F:−161) e lo confrontano col MB del referto InBody; bandierina 🚩 resa da `_affidabilitaHtml` SOLO se diverge >15% (soglia STRETTA, scelta da Fabrizio: becca OCR/refusi/misure storte senza disturbare per la muscolatura alta, dove il MB InBody supera legittimamente Mifflin). `crossCheck` in entrambi i return; zero effetto sul TDEE (test dedicato lo verifica). Serve peso, altezza, età (da `nascita`) e sesso: se manca un dato il cross-check è `null` e non appare. Test dedicati (`s2-tdee-mifflin-orario.test.js`), suite 77→85.
8. ✅ **Orario allenamento nel contesto AI** (24 lug 2026) — `costruisciContestoPaziente` ora aggiunge nel blocco Attività una riga "Orario allenamento: …" (con la nota se Variabile) e una direttiva peri-workout (carboidrati facilmente digeribili ~1-2 h prima, quota proteica + carbo nel pasto successivo, sempre compatibile col regime e col budget carbo → sicura anche in keto; giorni OFF distribuiti normalmente). Abilita i suggerimenti pre/post-workout dell'AI. Nessun effetto sul calcolo. Coperto da test (`s2-tdee-mifflin-orario.test.js`).
9. ↗️ **Previsione dimagrimento: da data secca a range** — **EVOLUTO IN P115** (24 lug 2026): nella sessione di design del 24/7 Fabrizio ha deciso di elevare questo passo a funzione completa — la scheda "📈 Percorso" (timeline di periodizzazione con proiezione ibrida). Il lavoro prosegue nella scheda P115; la vecchia data secca in `calcolaMacros` (~r9656-9699, 7700 kcal/kg lineare) verrà sostituita/affiancata dalla proiezione della Tappa 2 di P115. Questo passo non va più lavorato dentro P114.
...
**SCHEDA:** Stato: **CHIUSA COME VOCE** (24 lug 2026 — passi 1-8 ✅ tutti fatti e in produzione; il passo 9 è confluito in P115 dove prosegue come Tappa 2) · Priorità: Media · Categoria: Motore clinico · Dipendenze: nessuna · Origine: revisione critica proposte ChatGPT, 19 lug 2026.

### P115 — Scheda "📈 Percorso": timeline di periodizzazione (nata il 24 lug 2026 dall'evoluzione di P114 passo 9)
**COS'È:** scheda dedicata nella cartella paziente per pianificare e leggere le FASI del percorso (deficit 2-3 mesi → mantenimento 1-2 mesi → …, per non stressare tiroide/metabolismo) e vedere il peso reale muoversi dentro le fasi, con proiezione a cono al posto della data secca. Design completo (mockup + decisioni) nel doc di progetto Claude "NutriGest_P9_Timeline_Ragionamento": grafico INTEGRATO (bande fasi + peso sullo stesso asse tempo, corsia energia separata in kcal — mai due scale sovrapposte, lezione anti-fiber), proiezione IBRIDA (teorica che si ricalibra sul TDEE osservato), strati dati come interruttori (versione tecnica per Fabrizio / pulita per il paziente), predisposizione al futuro livello "Piano vs Realtà" (consuntivo sgarri/aderenza dall'app paziente P50 — slot previsto, non costruito).
**TAPPE:**
1. ✅ **Fondamenta** (24 lug 2026): linguetta "📈 Percorso" (tra TDEE e Note cliniche), modello dati `p.percorso={inizio, fasi:[{tipo, settimane, pct}]}` con fasi consecutive per costruzione, editor completo (aggiungi/modifica/riordina/elimina, alternanza suggerita deficit↔mantenimento), grafico SVG con bande per fase, fasi future tratteggiate, peso reale da `_serieePesoOss` (● InBody / ○ pesate), linea "oggi", linea obiettivo, badge fase corrente. kcal per fase indicative (% del TDEE via `calcolaTDEE`). Lezione fuso orario: date fasi SEMPRE via `_percorsoIsoLocal` (locale), mai `toISOString` (UTC → slittamento di un giorno in Italia). Test `s2-percorso.test.js`, suite 85→92.
2. ✅ **Proiezione ibrida** (24 lug 2026) — `_percorsoProiezione` (pura): dal peso reale più recente proietta giorno per giorno lungo le fasi; intake = TDEE stimato×(1+pct/100), bilancio = intake − TDEE di riferimento (OSSERVATO `tdeeOss` se disponibile → "calibrata", altrimenti stimato → "teorica"), pendenza = bilancio/7700. In mantenimento calibrato con osservato<stimato la pendenza diventa positiva (l'adattamento si vede); un "deficit sulla carta" sotto l'osservato viene smascherato. Cono ±(0.25 kg + giorni×TDEE×rangePct%/7700), rangePct dall'affidabilità o 8% se calibrata; obiettivo come intervallo dal(ottimista)/al(prudente). **Data secca di `calcolaMacros` sostituita** dall'intervallo (settimane min–max, "arrivo tra X e Y", nota "a ritmo costante" + rimando alla scheda). Solo informativa (test fissa zero effetti sui calcoli). Test `s2-percorso-proiezione.test.js`, suite 92→98.
3. ✅ **Corsia energia** (24 lug 2026) — grafico a due corsie (solo col percorso; senza fasi layout compatto invariato, fissato da test): sotto il peso la corsia "Energia (kcal)" con asse proprio. `_percorsoSerieEnergia` (pura): introito prescritto passato dagli slot `macrosStorico` (slot valido fino al successivo) + futuro dalle fasi (tratteggiato); TDEE stimato a gradini storici (campo `tdee` degli slot) + tratto attuale; TDEE osservato come segmento viola sul suo tratto [dal..al]. La distanza arancio↔verde/viola è il deficit/surplus reale nel tempo (il finto deficit si vede qui e nella proiezione che risale). Test `s2-percorso-energia.test.js`, suite 98→103.
4. ✅ **Composizione + interruttori** (24 lug 2026) — linea massa magra `_percorsoSerieMassaMagra` (solo InBody, quadratini magenta tratteggiati) sullo STESSO asse kg del peso: il divario tra le due linee È la massa grassa, nessun calcolo aggiunto. % grassa in tooltip nativo SVG (`<title>`, hover sul marker). Interruttori strati (checkbox: massa magra, cono proiezione, corsia energia, dettagli fasi %/kcal) + due preset: 🔧 Vista tecnica (default, tutto acceso) e 🙂 Vista paziente (nasconde massa magra/energia/dettagli, lascia peso+bande+cono). Stato SOLO di sessione (`_percorsoLayersState`, mai su `p`, mai salvato — è una preferenza di visualizzazione, non un dato clinico). Test `s2-percorso-composizione.test.js`, suite 103→111.
5. ✅ **Slot consuntivo (predisposizione)** (24 lug 2026) — CONTRATTO DATI `p.consuntivo={fonte, giorni:[{data, aderenza:ok|parziale|sgarro, extraKcal, opz. extraProt/Carb/Grassi, passi, sonnoOre}]}` fissato per P50 (schema in Contesto/STRUTTURA DATI) + motore puro `_percorsoConsuntivo` (% aderenza, extra kcal, ritardo = extra÷7700, settimane perse = ritardo ÷ ritmo pianificato — media pesata fasi deficit). Nel grafico: striscia aderenza 🟢🟡🔴 (un rettangolino/giorno, tooltip) tra corsia peso ed energia + riga riepilogo + interruttore (acceso anche in vista paziente: è il momento educativo). Oggi degrada a INVISIBILE per tutti (nessun paziente ha il campo; layout al pixel come Tappe 3-4, fissato da test): si accenderà da solo quando P50 scriverà i dati. Test `s2-percorso-consuntivo.test.js`, suite 111→118.
**FOCUS COMPONENTI COINVOLTI:** scheda paziente (nuova linguetta + pannello, righe ~6020-6280), riuso `_serieePesoOss`/`calcolaTDEE`/`calcolaTDEEOsservato`. Dati: nuovi campi `p.percorso` e `p.consuntivo` (contratto per P50) nel blob paziente (sync invariato).
**SCHEDA:** Stato: **IMPLEMENTAZIONE COMPLETA — tutte le 5 tappe ✅ 24 lug 2026** (resta il collaudo in produzione di Fabrizio, poi la scheda si archivia; il livello "Piano vs Realtà" si accende con P50, architettura già pronta) · Priorità: Media · Categoria: Motore clinico / UI · Dipendenze: solo P50 per accendere il consuntivo · Autonomia: L1 con test verdi prima/dopo e approvazione di Fabrizio a ogni tappa · Origine: evoluzione di P114 passo 9, sessione di design 24 lug 2026.
...
### P151 — Chiave primaria senza `user_id` su 4 tabelle: debito che scade quando vendi ✅ CHIUSA 11 agosto 2026
**IL FATTO (verificato sul database il 6 agosto 2026, non dedotto dal codice):** su `pazienti`, `collections` e `analisi_sangue` la chiave primaria e' la coppia `(id, user_id)` — corretta. Su **`ricette`, `piani`, `entrate` ed `eventi` e' il solo `id`**: lo spazio degli identificativi e' condiviso fra tutti i clienti presenti e futuri.
**PERCHE' NON E' UN ALLARME OGGI:** gli id nascono da `uid()` = `Date.now().toString(36)` + 4 caratteri casuali base36. Una collisione richiede **lo stesso millisecondo E gli stessi 4 caratteri** (1 su 1.679.616 a parita' di ms): fra due utenti ~6·10⁻⁷ per evento. E dentro lo stesso utente il rischio e' praticamente nullo, perche' `uid()` viene chiamata una volta per azione (una nota, un evento, un referto), mai in ciclo stretto — verificato sulle 19 chiamate.
**PERCHE' VA COMUNQUE SISTEMATO PRIMA DI VENDERE:** le scritture usano `Prefer: resolution=merge-duplicates`, cioe' un upsert. Se l'id collide fra due utenti, il `DO UPDATE` viene filtrato dalla policy `USING (user_id = auth.uid())`, non trova la riga dell'altro e **il record del secondo utente non viene salvato**. I dati del primo NON vengono sovrascritti — RLS protegge — ma il secondo perde il salvataggio, e non e' detto che se ne accorga. La probabilita' cresce col numero di clienti, e la migrazione costa poco adesso e molto su dati di clienti veri.
**LA CORREZIONE:** portare la PK a `(id, user_id)` come sulle altre tre tabelle, e allineare il target di conflitto dell'upsert. Da fare con Fabrizio, mai di iniziativa: e' DDL sul database di produzione, dove `git revert` non esiste.
**CHIUSA L'11 AGOSTO 2026, in un'unica migrazione con P152 e P154** (`p151_p152_p154_pk_composta_timestamptz_rls_initplan`, progetto `zrhmspylnlklppvhgplp`). Verificato **prima** di scrivere, e questo e' il motivo per cui non e' fallita: `user_id` e' NOT NULL con default `auth.uid()` su tutte e quattro e **zero righe l'avevano nullo**; nessuna foreign key puntava alle PK da sostituire. Verificato **dopo**: PK `(id, user_id)` su tutte e quattro, **zero id duplicati**, e conteggi righe identici (ricette 27, piani 36, entrate 22). **L'allineamento dell'upsert non e' servito**, ed e' la scoperta della passata: i payload verso quelle tabelle non dichiarano `on_conflict`, quindi PostgREST usa la PK della tabella — che ora e' quella nuova. *Una riga di codice che non c'e' e' una riga che non si puo' dimenticare di cambiare.* Nessuna modifica a `index.html`.
**SCHEDA:** Stato: ✅ Chiusa 11 ago 2026 · Priorità: Media (Alta se si avvicina la vendita) · Categoria: Backend / Supabase · Dipendenze: nessuna · Complessità 2 · Impatto 4 · Rischio 4 · Autonomia LLM: **L0** — eseguita con OK esplicito di Fabrizio nello stesso messaggio.

### P152 — `updated_at` di due tipi diversi: fragilità latente, non bug ✅ CHIUSA 11 agosto 2026
**IL FATTO:** `updated_at` e' `timestamptz` su `pazienti`, `ai_usage`, `collections`, `analisi_sangue` e **`text`** su `ricette`, `piani`, `entrate`, `eventi`. Su quel campo i dispositivi decidono chi ha salvato per ultimo (regola 15).
**VERIFICATO SUI DATI, NON SUPPOSTO:** le quattro colonne di testo sono popolate al 100%, tutte in formato ISO, **tutte lunghe esattamente 24 caratteri, un solo formato**. Il confronto fra testi ISO a lunghezza fissa ordina correttamente nel tempo: **nessun bug attivo oggi**.
**PERCHE' RESTA IN ROADMAP:** una colonna di testo non garantisce nulla. Basta un punto del codice che scriva una data formattata diversamente (senza millisecondi, con offset invece di `Z`, o una stringa locale) perche' il confronto smetta di ordinare — e il sintomo sarebbe una modifica che sparisce passando da un dispositivo all'altro, cioe' invisibile finche' non e' successo. E' lo stesso schema della famiglia F4/regola 15: **il tipo giusto nel punto di scrittura vale piu' di qualunque controllo a valle.**
**LA CORREZIONE:** convertire le quattro colonne a `timestamptz`. Non urgente; da fare insieme a P151 per non toccare due volte le stesse tabelle.
**CHIUSA L'11 AGOSTO 2026**, nella stessa migrazione di P151 e P154. La conversione non poteva fallire e lo si sapeva prima: controllate **tutte e 131 le righe** delle quattro tabelle (27+36+22+46), **zero fuori dal formato ISO a 24 caratteri**. Dopo: `timestamptz` su tutte e otto le tabelle del progetto, nessuna esclusa. Lato client nessuna modifica: su quelle tabelle `updated_at` serve solo a `order=updated_at.desc` (che il server ora ordina come date) e a un `new Date(...)` che legge qualunque ISO. *Il difetto non mordeva; e' stato chiuso perche' costava un minuto adesso e una modifica che sparisce fra due dispositivi dopo.*
**SCHEDA:** Stato: ✅ Chiusa 11 ago 2026 · Priorità: Bassa · Categoria: Backend / Supabase · Dipendenze: chiusa insieme a P151 e P154 · Complessità 1 · Impatto 2 · Rischio 3 · Autonomia LLM: **L0**

### P154 — Le policy RLS rivalutano `auth.uid()` a ogni riga: terzo debito che scade quando vendi ✅ CHIUSA 11 agosto 2026
**IL FATTO (letto dal Performance Advisor e dalle policy vere il 10 agosto 2026, non dedotto dal codice):** tutte e **8** le tabelle hanno una policy che chiama `auth.uid()` **una volta per riga** invece che una volta per query — `owner_all_pazienti`, `owner_all_piani`, `owner_all_eventi`, `owner_all_entrate`, `owner_all_ricette`, `owner_all_collections`, `ai_usage_select_own`, `analisi_sangue_row_owner`. Nessuna è nella forma ottimizzata; verificato con una query su `pg_policies`, non solo dall'avviso.
**IL CONTEGGIO GIUSTO NON È 8, È 15** (regola 16 — contare non è ancora il lavoro): sette policy sono `FOR ALL` e hanno **sia `USING` sia `WITH CHECK`**, la sola `ai_usage_select_own` è `FOR SELECT` col solo `USING`. Le espressioni da riscrivere sono quindi quindici, e una passata che ne corregge otto lascia il difetto per metà — invisibile, perché l'advisor smetterebbe comunque di segnalare la tabella.
**PERCHÉ NON È UN ALLARME OGGI, detto coi numeri:** l'intero database contiene **332 righe** (47 pazienti, 27 ricette, 36 piani, 22 entrate, 46 eventi, 111 ai_usage, 4 collections, 39 analisi). A questa scala la differenza non è misurabile e non lo sarà per molto tempo: **non c'è niente da correggere in fretta.**
**PERCHÉ VA COMUNQUE SISTEMATO PRIMA DI VENDERE:** la penalità cresce **col numero di righe**, cioè col numero di clienti. È lo stesso schema di P151 e P152 — *debito che scade quando vendi* — e riguarda **le stesse otto tabelle**.
**LA CORREZIONE:** sostituire `auth.uid()` con `(select auth.uid())` in tutte e quindici le espressioni. Più, separata e facoltativa: l'indice `ai_usage_user_created` risulta **mai usato** (avviso `unused_index`, livello INFO) — da rimuovere solo dopo aver verificato che nessuna query futura lo preveda; un indice inutilizzato costa poco, rimuoverne uno che serviva costa una scansione completa.
**DA FARE NELLA STESSA PASSATA DI P151 E P152**, e questa è la parte non negoziabile: sono **DDL sul database di produzione**, dove `git revert` non esiste e dove ci sono i dati sanitari di pazienti veri. Aprire tre volte le stesse otto tabelle è tre volte il rischio. La sequenza: SQL mostrato a Fabrizio **prima** di eseguirlo, `project_id` dichiarato a voce (l'organizzazione ne ha due), e riletta degli advisor **dopo** per verificare che gli avvisi siano davvero spariti — non per fiducia, per controllo.
**CHIUSA L'11 AGOSTO 2026**, nella stessa migrazione di P151 e P152 — la sequenza prescritta dalla scheda e' stata seguita per intero: SQL mostrato a Fabrizio prima di eseguirlo, `project_id` dichiarato, advisor riletti dopo. **Riscritte tutte e quindici le espressioni**, non le otto policy: sette `FOR ALL` con `USING` e `WITH CHECK`, piu' `ai_usage_select_own` col solo `USING`. Advisor dopo: **zero `auth_rls_initplan`**; resta il solo `unused_index` su `ai_usage_user_created`, che la scheda dava come separato e facoltativo e **non e' stato toccato**.
  **L'ERRORE DA NON RIPETERE, ed e' mio.** La prima query di verifica ha stampato «8 policy non ottimizzate» e per un momento la migrazione e' sembrata passata a meta'. Era sbagliata la **query**: Postgres normalizza `(select auth.uid())` in `( SELECT auth.uid() AS uid)`, e il mio criterio cercava `auth.uid()` non preceduto da parentesi — trovando lo spazio dopo `SELECT`. Lette le otto policy una per una, sono tutte nella forma giusta. *E' la stessa forma dell'errore sui «117 codici Compendium» del 10 agosto: contare col criterio sbagliato e credere al numero invece di guardare le righe.* **Un controllo che dichiara un difetto va verificato come si verifica un difetto.**
**SCHEDA:** Stato: ✅ Chiusa 11 ago 2026 · Priorità: **Bassa oggi, Media vicino alla vendita** · Categoria: Backend / Supabase · Dipendenze: chiusa insieme a P151+P152 · Complessità 1 | Impatto 2 | Rischio 4 (è DDL in produzione) · Autonomia LLM: **L0** — nessuna scrittura di iniziativa, mai.

### P157 — La sync spingeva tutto PRIMA di idratare: il difetto che ha sovrascritto sei pazienti ✅ CHIUSA E COLLAUDATA 12 agosto 2026
**IL FATTO (visto in diretta, log console 23:09 dell'11 ago 2026):** `syncNow` faceva `pull lista` -> **push di TUTTI i 41 pazienti coi blob in memoria** -> idratazione dei cambiati. L'ordine era rovesciato: chi aveva memoria vecchia riscriveva il server un istante prima di scaricare il nuovo. Cosi sono stati persi i referti di sei pazienti (recuperati dal backup del 9 ago — verbale nel CHANGELOG dell'11-12 ago), e cosi il primo ripristino e durato tre minuti.
**DOVE ERA DAVVERO, e non era dove sembrava:** il controllo conflitti P69 esisteva ed era giusto, ma viveva SOLO in `_flushDirtyIds` (push per-id). Il push COMPLETO era l'unica strada che scriveva senza passare da nessun controllo — ed e' la strada dell'avvio, del pulsante «Sincronizza» e di ogni `save()` senza id. *Una guardia che copre una strada su due e' una guardia che qualcuno aggirera' senza saperlo.*
**LA CORREZIONE, tre difese** (commit di consegna del 12 ago 2026, baseline `2b8b556`):
1. **Ordine** — `_pazIdrataCambiati` espone la sua promessa (`window._idratazionePromise`) e `pushToSheets` la attende come prima istruzione. L'attesa sta DENTRO `pushToSheets`, non nei tre chiamanti (regola 15, "ripara il rubinetto").
2. **Guardia per riga, con la reazione giusta** — `_p157Decidi` (funzione PURA) classifica ogni paziente. Server piu' nuovo **senza** modifiche locali = copia vecchia, si riscarica in silenzio; server piu' nuovo **con** modifiche locali = conflitto vero, dialogo a tre vie P69. Nell'incidente i sei pazienti erano tutti nel primo caso: la risposta giusta era scaricare, non chiedere.
3. **Impronta del contenuto** — si spinge solo chi e' cambiato davvero. Taglia 41 POST per sync (ognuna faceva avanzare `updated_at`, obbligando ogni altro dispositivo a riscaricare 41 blob) ed e' la seconda difesa dove P69 e' fail-open. **L'impronta da sola avrebbe fermato l'incidente.**
**INOLTRE:** `_dirtyIds` non si svuota piu' in blocco (le righe in conflitto non sono state scritte e devono restare sporche); la sincronizzazione d'avvio e' marcata `syncNow({avvio:true})` e a 800 ms fa **solo download** se non c'e' niente di sporco.
**SECONDA GAMBA — chiusa con un attrezzo, non con un controllo.** La freschezza si decide da `updated_at`, e una scrittura fatta DIRETTAMENTE sul database puo' lasciarlo invariato: e' successo col ripristino da backup, ed e' il motivo per cui 5 pazienti su 6 non si riscaricavano piu'. Nessun confronto di date puo' accorgersene, per costruzione. Rimedio dichiarato invece che finto: **`riallineaTuttoDalCloud()`** — pulsante «☁️ Riscarica tutto dal cloud» in Impostazioni -> Supabase, ignora baseline e impronta, lascia stare i pazienti con modifiche in attesa. *Quando un difetto non e' rilevabile, si da' all'utente il comando per ripararlo e lo si dichiara.*
**TEST: 23 nuovi** (`s2-sync-dispositivo-indietro.test.js`), suite 738 -> **761 verdi**. Fra questi **l'incidente riprodotto** con un server finto (PC a 0 referti contro server a 24: zero scritture, e i 24 referti tornano in locale) e 4 guardie strutturali che impediscono di togliere il rimedio per distrazione. Il file **dichiara in testa cosa non vede** (RLS, rete vera, e proprio il caso `updated_at` invariato).
**COLLAUDO SUL CAMPO: SUPERATO 4 PROVE SU 4** (12 agosto 2026, da Fabrizio sull'app in produzione, commit `095b6ed`). Le prove sono conservate qui perche' servono a ogni futura modifica di questa zona:
1. **Avvio** — la console dice «nessuna modifica locale in attesa: nessun push» e i 24 referti ci sono. ✅
2. **Dispositivo indietro** — modifica dall'iPhone, sync dal PC: la scheda **si riscarica da sola**, nessun dialogo. ✅ *(e' l'incidente dell'11 agosto, nella sua forma innocua)*
3. **Conflitto vero** — stesso paziente modificato su entrambi senza sync in mezzo: compare il dialogo a tre vie. ✅
4. **Riallineamento forzato** — «Riscarica tutto dal cloud» due volte di fila, conteggi invariati. ✅
**LA PROVA IN PIU', ed e' quella che ha insegnato qualcosa.** Fabrizio ha cancellato un referto e — **senza premere Sincronizza** — ha ripremuto «Riscarica tutto dal cloud»: sono rimasti 23, e ha chiesto se fosse giusto. Lo era, ma **c'erano due strade per arrivare a 23 e solo una era quella buona**: (a) la cancellazione era gia' sul server (`delInbody` chiama `save(p.id)`, che spinge da solo dopo il debounce di 2s) e il pulsante ha scaricato il valore vero; oppure (b) la scheda era ancora sporca, e il pulsante l'ha **saltata** — mostrando 23 senza aver scaricato niente. A schermo le due sono indistinguibili. Deciso con una query aggregata di sola lettura sul database (`jsonb_array_length(data->'inbody')`): **una sola scheda a 23, nessuna a 24** -> caso (a), il pulsante ha fatto il suo mestiere. *Il segnale che distingue i due casi c'e' gia' ed e' nella finestra di conferma («N schede con modifiche non ancora salvate NON verranno toccate»): se quella riga non compare, le schede saltate sono zero.* **Lezione: un esito giusto per il motivo sbagliato passa il collaudo lo stesso** — quando due meccanismi diversi producono lo stesso numero a video, il collaudo non e' finito finche' non si e' stabilito QUALE dei due ha agito (regola 16 applicata al collaudo invece che al conteggio).
**La tabella `_pre_ripristino_20260811` NON e' stata eliminata**: decisione di Fabrizio del 12 agosto, si tiene ancora qualche giorno perche' la sincronizzazione multi-dispositivo e' in uso vero solo da oggi. Non e' un residuo dimenticato, e' un'attesa voluta.
**IL BUCO TROVATO DAL COLLAUDO -> P158.** Guardando il dialogo a tre vie della prova 3, Fabrizio ha chiesto se avesse senso una scelta che tiene entrambe le versioni. Ha ragione, e c'e' di peggio: **nessuna delle tre scelte conserva la versione dell'ALTRO dispositivo** — «Esporta» salva solo la propria. -> voce **P158**.
**FOCUS COMPONENTI COINVOLTI:** `_p157Decidi`, `_p157Impronta`, `_p157SetImpronta`, `_p157DropImpronta`, `_p157RemotiUpdatedAt`, `riallineaTuttoDalCloud`, `_idratazioneAttendi`, `_pazIdrataCambiati`/`_pazIdrataCambiatiOra`, `pushToSheets`, `syncNow`, `_pushRigaPerId`, `_pazFetchBlob`, `_p69RisolviRicarica`.
**SCHEDA:** Stato: ✅ **CHIUSA E COLLAUDATA 12 agosto 2026** (codice + 4 prove sul campo, stesso giorno) · Priorita: MASSIMA (integrita dati clinici) · Categoria: Sync / Supabase · Dipendenze: nessuna (si appoggia a P68, P69, P74 B2) · Complessita 3 | Impatto 5 | Rischio 5 · Autonomia LLM: L1 — progettata con Fabrizio (tre decisioni prese da lui: quante tappe, push all'avvio, attrezzo di soccorso), nessun collaudo saltato

### P159 — «Vai al giorno» del validatore portava al giorno ma non lo sguardo ✅ CHIUSA 12 agosto 2026
**DA DOVE NASCE:** il collaudo della fase 2 di P94. Fabrizio: «se controllo la parte delle allergie e lo apro si apre la schermata ma se clicco su vai a non succede niente».
**LA DIAGNOSI, e vale più del rimedio.** Il primo sospetto era la famiglia già pagata tre volte (il ramo di rendering vivo lasciato indietro): sbagliato. Riprodotto in JSDOM col Generatore a pillole → funzionava. Verificato allora **sull'app vera, in console**: `_pianoGiornoCorrente` passava da **6 (🏆 Giorno gara) a 0 (Lunedì)**, modalità manuale spenta, `#piano-select-paz` presente, `_renderGiornoGen` definita, **nessuna eccezione** (le righe successive allo stesso incolla venivano eseguite). Il programma faceva tutto giusto.
**IL DIFETTO ERA NELLA PROMESSA DEL COMANDO.** `_validaVaiAlGiorno` cambiava il giorno attivo e ridisegnava, **senza muovere la pagina**. E il pannello si apre dal badge del validatore, che sta *sopra* il piano: alla chiusura si guardano i badge mentre il cambiamento avviene più in basso, fuori schermo. **Un comando che si chiama "Vai" e non porta lo sguardo da nessuna parte è indistinguibile da un comando rotto** — ed è stato dato per rotto. `scrollIntoView` compariva **una volta sola in tutto `index.html`**, e non lì.
**LA CORREZIONE (minima per scelta):** dopo il ridisegno la pagina scorre sul box del piano, e una notifica dichiara dove si è finiti — *«↪ Lunedì · Colazione»*. Lo scorrimento da solo non basterebbe: muto, lascerebbe comunque il dubbio di non aver premuto niente. I due pulsanti del pannello ora passano anche lo **slot**, non solo il giorno (su una colazione e una cena dello stesso giorno il solo nome del giorno non basta).
**TROVATO DAL TEST, NON DALL'OCCHIO:** la mappa delle etichette dei pasti esisteva in **tre copie** (pannello validatore, `apriPannelloRicette`, e quella nuova). Unificata in `_VALIDA_SLOT_LBL` — regola 15, «ripara il rubinetto». Il test che lo impedisce è nel file.
**CONSAPEVOLMENTE NON FATTA — l'evidenziazione della riga colpevole.** Accendere per due secondi la cella o la riga segnalata sarebbe più utile quando le violazioni sono molte, ma tocca il disegno del giorno. Decisione di Fabrizio: prima il rimedio minimo, questo resta qui. *Nota per chi lo farà:* le violazioni da cella portano già `cellaId`, quelle da riga-ricetta hanno `ricettaIdx`, ma le voci **non verificabili non hanno indice di riga** — va aggiunto in `validaPiano`, altrimenti su quelle si può evidenziare solo il pasto.
**TEST: 8 nuovi** (`s2-valida-vai-al-giorno.test.js`), suite 761 → **769**. Il file dichiara in testa cosa non vede: JSDOM non ha scorrimento né rendering veri, quindi verifica che lo scorrimento venga *chiesto* sull'elemento giusto e che la notifica dica il giorno giusto — non che a schermo si veda.
**SCHEDA:** Stato: ✅ Chiusa 12 agosto 2026 · Priorità: Bassa (ma alta in fiducia: un comando che sembra rotto fa dubitare del resto) · Categoria: Validatore clinico / interfaccia · Dipendenze: P61 (validatore) · Complessità 1 | Impatto 2 | Rischio 1 · Autonomia LLM: L2 — rimedio scelto da Fabrizio fra tre, collaudo a video da fare

### P158 — Il dialogo dei conflitti non conserva la versione dell'altro dispositivo 🔵 DA FARE (aperta 12 ago 2026, dal collaudo di P157)
**DA DOVE NASCE:** la prova 3 del collaudo di P157. Visto il dialogo a tre vie, Fabrizio ha chiesto «me le spieghi? e aggiungere una che salva entrambe le modifiche ha senso?». Mettendo le tre scelte in fila e' saltato fuori un difetto che nessuno aveva scritto: **`_p69RisolviRicarica` butta la versione locale, `_p69RisolviSovrascrivi` butta quella remota, e `_p69EsportaLocali` salva SOLO la propria.** Qualunque strada si scelga, una delle due versioni sparisce e **la versione dell'altro dispositivo non ha mai una copia**. Su un blob clinico e' la stessa famiglia dell'incidente dell'11 agosto, un piano piu' in alto: li' si perdeva senza saperlo, qui si perde premendo un pulsante — che e' meglio, ma non e' abbastanza.
**TAPPA 1 — LA RETE DI SICUREZZA (poco codice, toglie subito il rischio).** Prima di eseguire una qualunque delle tre scelte, l'app salva da sola un file con **entrambe** le versioni (locale e remota) dei record in conflitto. Nessuna decisione in piu' per l'utente, nessuna strada senza uscita di emergenza. **Da sistemare nella stessa tappa un limite gia' misurato:** il file prodotto oggi da `_p69EsportaLocali` e' un array nudo, e `importa()` pretende un oggetto con la chiave `pazienti` — quindi quel file **non e' reimportabile** e serve solo a essere guardato. O si scrive nel formato che `importa()` accetta, o si dichiara a schermo che e' una copia di consultazione: oggi non fa ne' l'una ne' l'altra cosa.
**TAPPA 2 — LA QUARTA SCELTA: «unisci quello che si puo' unire».** Non e' un merge generico, e la distinzione e' il punto della voce. Il record e' un blob salvato intero: sulle **caselle singole** (peso obiettivo, note, regime) se due dispositivi hanno scritto valori diversi non esiste un «entrambe» — sceglierne uno d'ufficio sarebbe inventare (regola 11/14). Ma sugli **elenchi che possono solo crescere** — `p.inbody`, `p.invii`, `p.analisiSangue`, gli appuntamenti — «entrambe» ha un significato esatto: **unione**, con chiave la data (o l'id) della voce. Ed e' proprio li' che il danno e' stato vero: *se questa fusione fosse esistita l'11 agosto, la scheda di Lilly non avrebbe perso un referto nemmeno scegliendo il pulsante sbagliato.* Le poche caselle singole che divergono si mostrano **affiancate, una per una**, invece di decidere in blocco per tutta la scheda.
**COSA NON VA FATTO:** un pulsante «unisci» che fonde in silenzio anche le caselle singole. Sarebbe un avviso che ha sempre ragione (regola 19) su dati clinici — e la fusione va **mostrata**: quante voci sono state aggiunte e da dove.
**DIPENDE DA UN CENSIMENTO, non da un'intuizione:** prima di scrivere codice, l'elenco degli array «solo in crescita» dentro il blob paziente si verifica sul codice, non si ricorda a memoria — se uno di quelli fosse invece un elenco che viene riscritto per intero, unirlo creerebbe duplicati.
**SCHEDA:** Stato: 🔵 Da fare · Priorita: Media-Alta (integrita dati clinici, ma il caso e' raro e oggi visibile) · Categoria: Sync / Conflitti · Dipendenze: P69 (dialogo), P157 (chiusa) · Complessita 2 (tappa 1) + 3 (tappa 2) | Impatto 4 | Rischio 3 · Autonomia LLM: L1 — la tappa 1 si puo' fare da sola, la tappa 2 si progetta con Fabrizio

### P155 — Guardia meccanica Contesto→codice: gli identificatori citati devono esistere 🔵 DA FARE (aperta 11 ago 2026)
**DA DOVE NASCE:** la domanda di Fabrizio «questo errore si è ripetuto altre volte?» dopo il buco del Contesto su P151/P152/P154. La verifica ha trovato che gli **identificatori** citati nel Contesto sono sani (115 citati, 4 assenti — tutti e quattro citati *in quanto inesistenti*, tipo «il campo si chiama `inizioAlim`, NON `inizioPiano`»), mentre a invecchiare sono le **frasi in prosa** (caso P150: il passo *«3 · Quanto in fretta»*, fuso il giorno dopo). Da qui la divisione: la penna la governa la **regola 25**, la parte meccanizzabile la governa questo test.
**COSA FA:** nuovo test nella suite (accanto a `s1-doc-allineata`): estrae dal Contesto gli identificatori citati fra apici (funzioni, id DOM, campi) e verifica che esistano in `index.html`. Le eccezioni legittime — nomi citati apposta come inesistenti — vivono in una lista `CITATI_COME_INESISTENTI` col motivo accanto, sul modello di `ORFANI_NOTI`.
**DUE COSE DA SCRIVERE DENTRO IL TEST, non negoziabili:** (1) **il rimedio è aggiornare la frase del Contesto, MAI cancellare la citazione** — un test che spinge a togliere riferimenti per farlo tornare verde produce documentazione più vaga, che è peggio del difetto; (2) **la dichiarazione del buco**: questo test NON vede le frasi in prosa — non avrebbe fermato P150. Un verde qui non significa «Contesto allineato», significa «identificatori vivi». (È la lezione della suite verde su FODMAP che non esercitava `FODMAP_PORZIONI`.)
**MISURATO PRIMA DI APRIRLA** (11 ago 2026): il controllo sugli identificatori dà 4 falsi positivi su 115, tutti classificabili — nasce quieto. Il controllo alternativo sulle **etichette di interfaccia** è stato provato e **scartato coi numeri**: 15 falsi allarmi su 16 (il `·` separa anche MET, commit e colori), e l'unico caso che dava per buono era proprio il difetto vero, vivo solo in un commento morto. Quella strada non va riaperta senza numeri nuovi.
**SCHEDA:** Stato: 🔵 Da fare · Priorità: Media · Categoria: Test-suite / Documentazione · Dipendenze: nessuna (la regola 25 è già in vigore) · Complessità 2 · Impatto 3 · Rischio 1 · Autonomia LLM: L2 (il test si scrive e collauda da solo; la lista eccezioni si mostra a Fabrizio)

### P156 — Bonifica incrementale del Contesto: le frasi di struttura riscritte come stato 🔵 DA FARE (aperta 11 ago 2026)
**DA DOVE NASCE:** stessa origine di P155. Misurato l'11 ago 2026: il Contesto ha **13 righe** che raccontano un «prima/dopo» e **3 note datate** in stile CHANGELOG su 3.792 righe — poco, ed è proprio perché è poco che conviene adesso: è la stessa curva dei debiti P151/P152/P154, costo che cresce con la dimensione del file e col numero di lettori (un collaboratore, la due diligence di un acquirente). **Il Contesto è l'unico file a cui una sessione crede senza difendersi**: le fotografie hanno il cartello, il CHANGELOG è dichiaratamente storia, questo dichiara il presente — il suo tasso d'errore va tenuto più basso degli altri, non uguale.
**COSA FA:** lettura del Contesto sezione per sezione; ogni frase di **posizione o struttura** si verifica contro `index.html` e si riscrive nella forma della regola 25 (stato + identificatore + provenienza breve); le transizioni «prima X, ora Y» si accorciano a stato, il movimento resta al CHANGELOG che già lo racconta. Il primo esempio della forma nuova è già nel file: il paragrafo P149/P150 su `#mac-strada-box`, riscritto l'11 ago 2026.
**COME NON VA FATTA, e questo è il punto della scheda:** **mai in una passata sola, mai mescolata a lavoro sul codice.** Riscrivere in massa il file a cui tutte le sessioni future credono è l'operazione documentale più rischiosa che esista in questo progetto: un errore introdotto qui si propaga a ogni sessione. Sessioni corte e dedicate, una manciata di sezioni per volta, ogni affermazione verificata prima di riscriverla, commit piccoli. Dove una struttura si può **generare invece che asserire** (modello INDEX.md / `rigenera-index.js`), si preferisce la generazione: un documento generato non invecchia.
**SCHEDA:** Stato: 🔵 Da fare · Priorità: Bassa oggi, Media pre-vendita · Categoria: Documentazione · Dipendenze: regola 25 (in vigore); P155 utile prima, non obbligatoria · Complessità 2 (ma distribuita) · Impatto 3 · Rischio 2 (il rischio È il rimedio fatto male) · Autonomia LLM: L1 — ogni sezione riscritta si mostra a Fabrizio in diff

### P150 — Schermata TDEE: passi 1-2 riorganizzati, testi asciugati ✅ CHIUSA 6 agosto 2026
**IL PROBLEMA (misurato, non di gusto):** la pagina renderizzata a 1090 px era alta **2.573 px** — 3,7 schermate sul portatile — con **45 campi**, **26 pulsanti** e sei tinte di sfondo. Mescolava a peso visivo identico quello che si COMPILA, quello che l'app CALCOLA e quello che si CONSULTA; il risultato era sparso su tre punti della pagina, coi macro in fondo dietro «Calcola».
**PASSO 2 — da quattro campi a una domanda:** «Portare il grasso dal 28.5% al [15] %» con la sintesi accanto. Modo del muscolo, quota e guadagno scendono nella piega «Altro» con lo storico — P123 aveva già misurato che quella previsione sposta il peso finale di chili ma il grasso da togliere di due etti, quindi non decide né calorie né settimane. **«Deciso da» rimosso** su richiesta di Fabrizio: tolte anche le due letture di `trg-deciso`, non lasciate orfane.
**IL RISULTATO NON È PIÙ IL PESO IN GRANDE:** riquadro verde con le due coppie che contano (grasso, massa magra) e il peso come loro somma. **Un pulsante solo**, ancorato allo scenario a **massa magra invariata** (`r.ottimista`) perché non contiene previsioni; spento quando il traguardo a video è già quello salvato. Lo scenario col muscolo resta nella piega col suo «usa questo invece».
**PASSI 2 E 3 FUSI, E LA TABELLA DIVENTA UNO SLIDER:** erano una decisione sola spezzata in due. «Come ci arrivi» era una tabella a cinque colonne; ora è uno slider 0-40% le cui **tacche portano l'esito** (`0% · −10% · −15% · −20%` e sotto 53 / 36 / 27 settimane): la scala è diventata il confronto. Sotto il pollice un'etichetta viva. Lo stesso comando confronta e regola, quindi «Altra percentuale» sparisce e la piega «Regola a mano» nasce chiusa quando le tacche ci sono.
**LA TRAPPOLA DELL'ANTEPRIMA:** `_stradaUsa` mostra una notifica e apre un `confirm()` sulle fasi; agganciarlo a `oninput` avrebbe aperto decine di finestre per trascinamento. L'anteprima è separata dall'azione — `_ritmoAnteprima` non tocca il regime, `onchange` sì. Verificato: 30 movimenti, zero dialoghi.
**IL TRANELLO DI P149, RIPETUTO E RIBLOCCATO:** chiudendo la piega «Regola a mano» nel markup, un paziente senza sesso o senza InBody restava **senza alcun comando visibile per le calorie** (`_traguardoAnteprima` esce prima di disegnare lo slider). Il test P149 l'ha bloccato: la piega nasce aperta e la chiude `_aggiornaStradaBox` solo quando le tacche esistono.
**TESTI: 425 → 218 parole**, tre regole — una frase resta solo se dice qualcosa che i numeri accanto non dicono già; le sigle si sciolgono dove si spiega il metodo, non nelle etichette (`MB+NEAT+EAT+TEF` → «basale + passi + allenamento + digestione»); i titoli dicono cosa esce («Da dove parte» → «Quanto consuma»).
**COLLAUDO:** suite **738 verdi**, `node --check`, INDEX.md rigenerato, render vero in Chromium sul paziente di collaudo. Due test **aggiornati, non indeboliti**: il marcatore della strada in vigore passa dal pallino ◉ alla tacca `.near` (con due controlli in più sul pollice), e la nota di ricomposizione si cerca su «non la bilancia» invece che su «il peso dice poco» — cambia la formulazione, non l'intento.
**CONSAPEVOLMENTE NON FATTO — la forma «costruire muscolo» del passo 2.** Verificato sul codice: oggi per un percorso in surplus `_stradeVerso` esce con «nessun grasso da togliere» e il blocco delle strade **non compare affatto**; l'unico comando resta lo slider manuale. Serve: due campi nuovi in `obiettivoPercorso.clinico` (massa magra obiettivo, tetto di grasso), un ramo surplus in `_stradaCalcola`/`_stradeVerso`, migrazione sui pazienti salvati. **Deciso con Fabrizio che su quel percorso NON si stima il tempo**: il ritmo di crescita muscolare non si ricava dalle calorie (regola 11 — un valore inventato è peggio di uno mancante). Restano aperti anche i testi dei passi 3-4, le note condizionali del passo 1 e la destinazione dei «Riferimenti».
**FOCUS COMPONENTI COINVOLTI:** Frontend, `renderPdMacros`, `_traguardoPanelHtml`, `_traguardoAnteprima`, `_traguardoUsa`, `_traguardoAllineaManuale`, `_stradeHtml`, `_stradeEvidenzia` (riscritta), `_ritmoAnteprima` (nuova), `_stradaUsa`, CSS `.rit*`.
**SCHEDA:** Stato: ✅ **CHIUSA E COLLAUDATA** — chiusa 6 agosto 2026, **collaudata a video il 10 agosto 2026** sull'app in produzione (Claude in Chrome, sola lettura, nessuna scrittura: passi numerati in ordine, piega «Regola a mano» corretta su entrambi i casi, slider con anteprima su `input` e scrittura su `change`) · Priorità: Alta · Categoria: Motore TDEE / interfaccia · Dipendenze: P149, P147, P123, P122 · Complessità 3 · Impatto 4 · Rischio 2 · Autonomia LLM: L1

### P149 — Sezione TDEE, parte 2: dall'obiettivo peso in giù ✅ CHIUSA 5 agosto 2026
**IL PROBLEMA:** la stessa frase di P147 — «non ci capisco più nulla» — sulla seconda metà della scheda Macros. Tre difetti misurabili, non di gusto.
**(1) LE DECISIONI ERANO IN ORDINE SBAGLIATO.** Giorni di carico, ciclizzazione carboidrati e cronotipo stavano *prima* del calcolo: la ciclizzazione chiedeva «kcal giorni ON / OFF» mezza schermata prima che le calorie esistessero. **Due numeri che devono uscire dal calcolo, digitati a occhio.**
**(2) LE CALORIE SI DECIDEVANO IN SEI POSTI:** slider, campo kcal, campo %, dieci preset, tre «Usa» delle strade, «Ritara a X kcal». E dopo aver premuto «Usa» su −20% la tabella restava identica: **nulla diceva quale fosse quella in vigore.**
**(3) IL PESO OBIETTIVO IN TRE POSTI CHE SI CONTRADDICONO** — campo libero, pannello 🎯, chip di riferimento. Lo diceva il codice stesso: dal 26 luglio esisteva un avviso scritto apposta per quando campo e pannello divergono. **Un avviso che ripara una contraddizione creata dall'interfaccia è un sintomo, non una cura.**
**LA CORREZIONE:** cinque passi numerati — 1 Da dove parte · 2 Dove vuole arrivare · 3 Quanto in fretta · 4 Come si compone il piatto · 5 Come si distribuisce nella settimana. Campo peso a mano e quattro riferimenti sotto la piega «Altri modi»; verifica al controllo e strade spostate in `#mac-strada-box` accanto al regime che impostano; righe delle strade cliccabili con quella attiva marcata; slider/kcal/%/preset sotto «Regola a mano»; tabella per categoria richiudibile. **Il motore non è stato toccato**: cambia dove stanno le cose e quante volte compaiono.
**LA MARCATURA SI LEGGE, NON SI RICORDA.** `_stradeEvidenzia` rilegge il campo % del regime ed è agganciata a `_aggiornaRegimeSlider`, l'unico imbuto da cui passano slider, campi, preset, «Usa» e «Ritara». Cambiate le calorie da qualunque parte, la riga si spegne e compare «impostato a mano». Stessa lezione di P147c: *un numero visibile che contraddice i comandi visibili sopra di esso è peggio di un numero assente.*
**IL TRANELLO EVITATO:** con slider e preset dentro una piega chiusa, un paziente senza traguardo calcolabile (manca sesso o InBody) sarebbe rimasto **senza alcun comando visibile per le calorie**. La piega si apre da sola quando le strade non ci sono, una volta per apertura scheda (`_regimeFoldAuto`). Due test, uno per caso.
**IL CAMPO `mac-peso-target` NON È STATO RIMOSSO** — lo leggono salvataggio macros, `_traguardoUsa`, `_traguardoAllineaManuale`, `_usaRifPeso`, il salvataggio anagrafica con `_stessoPaz` e la validazione campi. Toglierlo sarebbe stata la famiglia F6/F7 (campo tolto, lettura rimasta, dato azzerato in silenzio).
**COLLAUDO:** suite 732 → **738** (`s2-tdee-parte2-ordine.test.js`); `node --check`; INDEX.md rigenerato; render vero in Chromium sul paziente dello screenshot. Riordino eseguito da uno script con ancoraggi verificati, non a mano: su 150 righe di markup dentro un template literal un taglio sbagliato non dà errore di sintassi, dà un `<div>` non chiuso che si vede solo a video.
**CONSAPEVOLMENTE NON FATTI** (decisione di Fabrizio: prima si guarda il riordino sui pazienti veri): il controllo di coerenza fra sedute dichiarate e giorni di carico spuntati, e le kcal ON/OFF della ciclizzazione proposte dal target invece che digitate. Restano la prima cosa da riprendere su questa zona.
**FOCUS COMPONENTI COINVOLTI:** Frontend, `renderPdMacros`, `_renderRifPesoBox`, `_traguardoAnteprima`, `_stradeHtml`, `_aggiornaStradaBox` (nuova), `_stradeEvidenzia` (nuova), `_aggiornaRegimeSlider`.
**SCHEDA:** Stato: ✅ **CHIUSA E COLLAUDATA** — chiusa 5 agosto 2026, **collaudata a video il 10 agosto 2026** insieme a P150 (stessa schermata; i cinque passi di P149 sono oggi quattro perché P150 ha fuso il 2 e il 3) · Priorità: Alta · Categoria: Motore TDEE / interfaccia · Dipendenze: P147 (parte 1), P122 (traguardo), P123 (strade), P127 (verifica al controllo) · C: 3 | I: 4 | R: 3 · Modello: Opus High · Autonomia: L0 (dati clinici).

### P147 — Sezione TDEE, parte 1: attività fisica → LAF ✅ CHIUSA 3 agosto 2026
**IL PROBLEMA:** Fabrizio, che l'ha progettata lui, non capiva più il pannello. Due flussi (generico e preciso) visibili insieme quando se ne usa uno solo; la sigla interna «P7» in interfaccia; un solo allenamento per paziente; valori MET fermi al Compendium 2011 sotto un commento che diceva 2024.
**IL MOTORE NON ERA IL PROBLEMA.** `calcolaTDEE` aveva già dentro la priorità giusta (sedute+minuti battono le ore; attività specifica batte tipo+intensità) — solo che non la dichiarava mai. Nello screenshot: Tipo e Intensità *non specificati*, calcolo a 8 MET lo stesso, nessuna spiegazione. **Quando l'utente dice «non ci capisco nulla» su una cosa che ha progettato lui, il difetto di solito non è nella formula ma in ciò che la formula non racconta di sé.**
**IL DIFETTO VERO — un solo allenamento per paziente.** Col paziente 2 pesi + 2 CrossFit + 1 corsa, qualunque singola attività si scelga è sbagliata: a parità di monte ore (5×45 min, peso 77.8) la realtà fa 261 kcal/giorno, «tutto pesi» 208, «tutto corsa» 346. **85 kcal/giorno, nella direzione peggiore** — più cibo di quanto abbia bruciato; su un ipocalorico da −500 è oltre un sesto del deficit. **La correzione:** l'allenamento è una lista di righe `{nome, sedute, minuti}` e l'EAT è la somma. La formula non cambia, è lo stesso `(MET−1)×peso×ore` dentro un ciclo.
**I MET ERANO DEL 2011.** Pilates 3.5→1.8 · Spinning 7.0→9.0 · Circuit training 8.0→7.5 · HIIT 8.0→7.0/11.0 · Yoga 3.0→2.3 · Corsa 10 km/h 10.0→9.3. Catalogo 30→97 voci (88 attività + 9 di ripiego), ognuna col **codice ufficiale** del Compendium accanto al MET (l'etichetta italiana è una traduzione nostra, il codice no). *Regola 20: un commento che cita una fonte non è una verifica che i numeri vengano da quella fonte.*
**IL CROSSFIT NON ESISTE NEL COMPENDIUM** — e non è una dimenticanza: non è un'attività a intensità costante. La letteratura misura i WOD (~12.9 MET su «Karen»), ma su 8-12 minuti; **il motore moltiplica per i minuti effettivi, quindi 12 MET su 60 minuti sovrastimano di ~250 kcal a seduta.** Scelta: voce «CrossFit / functional training (seduta intera)» 7.5 MET (proxy 02040) **marcata come stima**, più «HIIT / WOD vigoroso (solo tempo di lavoro)» 11.0 per chi dà i minuti del solo WOD.
**«NON SI ALLENA» ORA SI PUÒ DIRE.** Prima campi vuoti significavano insieme «non si allena» e «non gliel'ho chiesto». Ora la modalità è esplicita e l'affidabilità distingue i due casi: **il TDEE non cambia, cambia quanto ci fidiamo.** Il bonus scatta solo se la modalità è scritta davvero, non quando è dedotta da un paziente storico senza campi.
**BUG SILENZIOSO TROVATO STRADA FACENDO (4ª della famiglia `pesoTarget`):** il salvataggio dell'anagrafica leggeva i campi attività, che però vivono nel pannello TDEE. Col pannello chiuso li **azzerava**; col pannello aperto su un altro paziente **glieli copiava addosso**. La guardia `_stessoPaz` esisteva già dieci righe sopra per `pesoTarget` e non era stata estesa. **Una guardia scritta per un campo solo è una guardia che verrà dimenticata al secondo campo.**
**MIGRAZIONE:** le etichette storiche risolvono via `_MET_ALIAS`. **Rinominare una voce di catalogo è una migrazione di dati, non un ritocco di testo:** senza alias il MET sarebbe diventato null, la riga scartata, l'EAT zero — senza errori a video. Un test percorre tutte e 28 le etichette.
...
**PERCHÉ CONTA:** il flusso di prenotazione (telefonata → profilo → messaggio di preparazione con data E ora) ha bisogno di un appuntamento con orario. Il messaggio di preparazione NON è utilizzabile in pieno finché questa voce è aperta.
**LA SCOPERTA (30 lug, 6ª sessione) — la forma del problema era diversa da come è scritta qui sopra.** Le tre fonti non descrivono la stessa cosa. **Quattro delle cinque date di `dateCalendario` (primo, chiamata sett.1, msg sett.2, msg sett.3) NON sono appuntamenti**: sono promemoria per il nutrizionista, derivati dalla data di inizio piano. Nessuno si presenta in studio, un orario non serve e non servirà mai. Gli unici due fatti che sono appuntamenti veri sono la **prima visita** e il **controllo**. Quindi non era una tripla fonte su tutto: era una **doppia fonte sulle due sole cose che sono appuntamenti** — problema più piccolo, e con la soluzione già dentro: delle due ipotesi che questa scheda proponeva, vince la **separazione onesta** (dateCalendario = pianificazione, db.eventi = calendario), non la fusione.
**IL SECONDO DIFETTO, EMERSO DAL RACCONTO DELLA PROCEDURA REALE:** `salvaPaz` scriveva `controllo: g('p-controllo') || addDays(ini,28)`. Ma Fabrizio il controllo **lo concorda col paziente** — alla prima visita o alla chiamata della settimana dopo — e cade a 14 giorni per i keto, o a 25, o a 32. Quel +28 non era un default ragionevole: era **un appuntamento inventato** che finiva in calendario come se fosse fissato, e su cui la dashboard costruiva l'avviso «controllo saltato» contando i giorni da una data mai concordata con nessuno. È la **regola 11** del CLAUDE.md alla lettera (terzo caso della famiglia, dopo `salvaInbody` e P118).
**LA SOLUZIONE ADOTTATA:** `db.eventi` è l'unico posto dove NASCE un appuntamento. `p.visitaData` / `p.controlloData` restano come **specchio leggibile** (schema di P118 tappa 1 e di `pesoTarget`), perché una decina di punti li legge già — timeline, scadenze dashboard, «paziente sparito», calcolo inizio piano. Nessun campo tolto.
**IL VINCOLO NON OVVIO — l'id deve essere DETERMINISTICO** (`anag-<tipo>-<idPaziente>`, non `uid()`): (1) PC e iPhone eseguono la migrazione ognuno per conto proprio, e con un id casuale nascerebbero due eventi diversi per la stessa visita — al primo sync il doppione tornerebbe dalla porta di servizio; (2) spostare l'appuntamento cambia la DATA della stessa riga, quindi l'ora già fissata non si perde.
**IL QUARTO PUNTO D'INGRESSO:** oltre ai tre canonici della regola 12 (load, blob server, import), la migrazione va rigirata in **`pullEventiSupabase`**, che SOSTITUISCE `db.eventi` col contenuto del server: senza, un dispositivo già migrato vede sparire le visite dal calendario appena si sincronizza con uno non ancora migrato.
**LA TRAPPOLA DELLO SPECCHIO:** cancellare l'evento senza spegnere il campo del paziente lo fa **RICREARE** dalla migrazione alla ricarica dopo — un appuntamento cancellato che risorge. `delEvento` ora spegne anche il campo.
**DECISIONI CHIUSE CON FABRIZIO (30 lug):** appuntamenti storici migrati **tutti, senza ora** (nessun orario convenzionale: sarebbe un dato inventato); il **+28 sparisce** e in cambio l'avviso «controllo saltato» diventerà «controllo **da fissare**» in Tappa 2; il punto d'ingresso dell'ORA nell'interfaccia **si decide provando** (campo-ora in anagrafica vs bottone «Fissa appuntamento») — Tappa 2 si apre con un mockup a confronto, stesso metodo di P145.
**COLLAUDO TAPPA 1:** banco deterministico a tavolino, 23 asserzioni su 9 scenari (migrazione storica, idempotenza a due giri, zero doppioni, due dispositivi in parallelo → stesso id, spostamento data con ora conservata, evento manuale che vince, cancellazione che spegne il campo, controllo vero con orario, paziente senza date). Tutte verdi. `node --check` OK.
**REGALI GRATIS DELL'UNIFICAZIONE:** il KPI «appuntamenti della settimana» contava solo `db.eventi` e ignorava le visite scritte in anagrafica — ora le conta; un appuntamento nato dall'anagrafica non aveva `id` e dal calendario non era né spostabile né cancellabile — ora sì.
**DAL COLLAUDO IN CHROME (31 lug 2026, commit successivo):** due difetti corretti subito. (a) `salvaEvento` non avvisava lo specchio, quindi creando l'evento con l'ora sullo stesso giorno ne comparivano DUE fino al ricaricamento — **lezione: uno specchio va agganciato anche a ogni punto in cui l'utente scrive dall'ALTRO lato, non solo ai punti d'ingresso dati; la migrazione ripara, ma ripara tardi, e "tardi" su uno schermo vuol dire "rotto"**. (b) `MESSAGGI.controllo` era il testo fisso «previsto per **oggi**»: aprendo il 31 lug il controllo del 26 ago si leggeva "oggi". Difetto preesistente reso visibile da P140. Ora `{data}` sostituito da `_evTestoPromemoria()` con la data vera; senza data la frase resta generica invece che sbagliata — regola 11 applicata al testo.
**TAPPA 2 — CHIUSA 31 luglio 2026.** Nata da un mockup a confronto (metodo P145) che doveva solo far scegliere fra «campo ora» e «bottone»: provandolo Fabrizio ha fatto emergere un difetto che nessuna delle due opzioni riguardava — *«come abbiamo tolto la regola dei 28 giorni, dovremmo togliere anche quella della chiamata dopo una settimana: i pazienti mi indicano il giorno e anche l'ora»* — e ha chiesto di semplificare «i molti campi che fanno le stesse cose». **Lezione di metodo: mettere in mano all'utente una cosa che funziona vale più che descrivergliela.** Cosa è stato fatto:
- **Le tappe non si salvano più, si calcolano** (`_appTappe`). `p.dateCalendario` teneva cinque date tutte derivate (`primo` era la copia identica di `inizioAlim`) — dismesso con migrazione. *Trappola:* `_pazPreservaCampi` (protezione F7) faceva RISORGERE il campo a ogni salvataggio — **una protezione generica va riletta quando si RIMUOVE un campo, non solo quando se ne aggiunge**.
- **L'ancora è l'ultimo appuntamento avvenuto**, non l'inizio piano. Elimina un buco mai notato: con l'ancora vecchia, dopo il primo mese non nasceva più nessun promemoria e il calendario taceva. Una tappa concordata (evento vero con giorno e ora) fa sparire quella proposta.
- **Specchio inverso** (`_appSpecchioInverso`): i campi della scheda sono una PROIEZIONE di `db.eventi` — «prima visita» = la più vecchia, «prossimo controllo» = il più in là. Si chiama **solo da gesti espliciti**, mai dai percorsi di caricamento (stessa disciplina di `propagaCancellazione`).
- **Lo specchio si ritira o viene PROMOSSO** (`_appRitiraSpecchio`): stesso giorno → si ritira; giorno diverso → diventa evento normale invece di essere cancellato. **Su dati clinici non si butta via niente per far tornare un modello.**
- **Via i campi doppi:** la linguetta «Calendario» conteneva gli stessi due campi della linguetta Dati con nomi diversi, più le funzioni che li ricopiavano. Rimossa; l'anteprima ora chiama `_appTappe()`, la stessa funzione del calendario, quindi non può mostrare date diverse da quelle disegnate.
- **L'ora: «la pulizia di A col controllo di B»** (parole di Fabrizio). Campo ora accanto alle due date + bottone «… altro» che apre la finestra evento completa. **L'ora non si memorizza sul paziente:** si legge e si scrive sull'appuntamento, altrimenti si riapriva la doppia fonte.
- **Avviso «📅 Controllo da fissare»** al posto di «controllo saltato»: domanda vera e verificabile (c'è un controllo fissato da qui in avanti?) invece di giorni contati su una data inventata. Chiave nuova, così i vecchi «gestito» non lo zittiscono.
**COLLAUDO:** 12 test nuovi (23 nel file), suite 433/433, INDEX rigenerato, e il rendering **guardato** a 545px e 397px (P145) — a mezza colonna il bottone «… altro» lasciava un buco, corretto.
**FOCUS COMPONENTI COINVOLTI:** Frontend + struttura dati paziente + migrazione.
**SCHEDA:** Stato: ✅ Chiusa (T1 30 lug, T2 31 lug 2026) · Priorità: era Alta — **P142 è sbloccata** · C: 3 | I: 3 | R: 2 · Modello: **Opus High + Thinking ON** (dati pazienti + migrazione) · Autonomia: L2.

### P141 — `today()` restituisce ieri fra mezzanotte e le 2:00 ✅ CHIUSA 31 luglio 2026
**IL PROBLEMA:** `today()` era `new Date().toISOString().slice(0,10)`. `toISOString` risponde con l'orologio di **Greenwich**: in Italia (avanti di 1h d'inverno, 2h d'estate) fra mezzanotte e le 01:00/02:00 locali Greenwich è ancora al giorno prima. Verificato eseguendo il codice vero con `TZ=Europe/Rome`: alle 00:30 del 12 agosto rispondeva **11 agosto**.
**IMPATTO:** basso in frequenza, alto in insidiosità — e la parte che conta è che quelle date finiscono **SALVATE** (registro invii, log consumi AI) e il giorno dopo non si correggono da sole. Regola 11 pura.
**IL SECONDO DIFETTO, NON PREVISTO DALLA SCHEDA:** anche `addDays` era rotto. `new Date(s)` legge la stringa come mezzanotte UTC ma `setDate` fa l'aritmetica in ora **locale**: se in mezzo cambia l'offset si perde un giorno. `addDays('2026-03-25',7)` rispondeva 31/03 invece di 01/04. **Silenzioso e stagionale:** ogni anno, per le settimane a cavallo di fine marzo, tutte le tappe automatiche e le date del piano uscivano un giorno prima. Ora l'aritmetica è tutta in UTC (`Date.UTC` + `setUTCDate`): calendario puro, nessun fuso di mezzo.
**LA SCOPERTA CHE HA DATO FORMA ALLA CORREZIONE:** nel file c'erano **TRE** funzioni per la stessa cosa — `today()` (rotta) e due copie corrette scritte da chi aveva sbattuto contro il problema nel proprio angolo: `_percorsoIsoLocal` (P118, 22 usi) e `_calYmd` (30 lug, 5 usi). **È F4 applicato a una FUNZIONE invece che a un campo:** il rubinetto restava rotto e ognuno metteva un secchio sotto il proprio pezzo di codice. Le due copie sono state rimosse; i 27 usi puntano all'unica `ymdLoc()`.
**LA PRECISAZIONE CHE HA EVITATO UN DANNO:** la scheda diceva «45 usi da sostituire». Sono 43, e **~23 sono giusti così**: i marca-tempo (`updated_at`, `creato`, `timestamp`) non dicono «che giorno è» ma «in quale istante preciso», ed è su quell'istante che due dispositivi decidono chi ha salvato per ultimo — sostituirli avrebbe **rotto la sincronizzazione**. **Lezione: una voce di roadmap che conta le occorrenze di un pattern non ha ancora fatto il lavoro; il lavoro è classificarle.**
**PUNTI CORRETTI:** `_inviiRegistra`, `_aiLogUsage`, nome file PDF su storage, filtri e KPI entrate (settimana e mese — all'una di notte del primo del mese si vedeva ancora il mese scorso), dashboard (oggi e confini della settimana). Due punti restano in UTC **di proposito** e ora lo dichiarano sulla riga.
**LA RETE DI SICUREZZA** (`test-suite/test/s1-date-locali.test.js`): il test diventa ROSSO su qualunque riga che ricavi un GIORNO da `toISOString`, a meno che la riga porti `/* UTC-VOLUTO: motivo */`. **La dichiarazione sta accanto al codice che deroga, non in un elenco dentro un file di test:** la legge chi modifica quella riga. Il test fissa anche che la funzione data-locale sia **una sola** — la quarta copia nasce rossa.
**DATI GIÀ SALVATI SBAGLIATI:** si lasciano (decisione di Fabrizio, 31 lug). Non si può sapere quali record furono scritti fra mezzanotte e le due, e correggerli tutti sposterebbe anche quelli giusti. **Un dato sbagliato che sai essere sbagliato è meno pericoloso di una correzione che ne sposta cento a caso.**
**FOCUS COMPONENTI COINVOLTI:** Trasversale.
**SCHEDA:** Stato: ✅ Chiusa 31 luglio 2026 · Priorità: era Media · C: 2 | I: 2 | R: 2 · Modello: Opus High · Autonomia: L1.
...
**SCHEDA:** Stato: ❌ **ESCLUSA definitivamente** (decisione di Fabrizio, 14 lug 2026) · Priorità: — (non si esegue) · C: 3 | I: 4 | R: 2 · Modello: — · Autonomia: —. **Non sblocca più nulla:** una voce esclusa non può essere il prerequisito di nessuno. P80 è stata **ripensata senza** il dedupe fuzzy (chiusa parziale, Strada A, 14 lug 2026, commit `bd1744f`), **P84** è stata **chiusa il 17 lug 2026 senza P37**, e **P3** resta aperta ma va progettata **assumendo che le 1.256 ricette non arriveranno mai**.
> 🔧 **Correzione (verificato 4 ago 2026).** Questa SCHEDA diceva «Stato: Da fare · Priorità: Bassa→Media (sblocca P3/P84/P80)» mentre il **titolo della stessa voce**, tre righe sopra, diceva «❌ ESCLUSO 14 luglio 2026»: titolo e scheda si contraddicevano, e la scheda — che per regola del file è quella che fa fede — era la parte sbagliata. Prova nel CHANGELOG (sessione del 14 lug 2026): *«P37 (caricamento 1.256 ricette dagli appunti) → ESCLUSO definitivamente dalla roadmap, giudicato spreco di risorse da Fabrizio. Impatto: P80 e P3/P84, che lo citavano come sblocco, vanno ripensati senza quel prerequisito»*. Verificato anche nel file stesso: `grep -n "^### P80\|^### P84" NutriGest_Roadmap_v4.md` → **P80 «✅ CHIUSA PARZIALE (Strada A) 14 luglio 2026, commit `bd1744f`»** e **P84 «✅ CHIUSA (17 lug 2026)»** — cioè due delle tre voci che P37 «sbloccava» si sono chiuse **senza di lei**, il che da solo smentisce la dipendenza.
...
**✅ GIROVITA E WHtR: GIÀ IN PRODUZIONE — NON RIFARLI (verificato 4 ago 2026).** La richiesta scritta qui sotto come «da non perdere» era **già soddisfatta dal codice quando è stata scritta**: il campo e l'indice esistono da fine giugno 2026. Verifica al commit `c9fffac`: `grep -n "ib-girovita" index.html` → l'input `<input type="number" id="ib-girovita">` con label **«Girovita (cm)»** nel form InBody, il campo nella lista di reset, il salvataggio `girovita:gn('ib-girovita')` e i limiti di plausibilità `{min:40,max:200,warn_min:50,warn_max:160,unit:'cm'}`; `grep -n "whtr" index.html` → in `CALCOLI_CLINICI` la voce `{id:'whtr', nome:'WHtR (vita/altezza)', deps:['_girovita','_altezza'], formula:(v)=>v['_girovita']/v['_altezza']}` con **soglie `{max:0.5 verde}` / `{max:0.55 giallo}` / rosso** e il testo *«Cutoff universale 0.5 valido per uomo, donna ed etnie diverse»* — cioè esattamente il cutoff 0,5 chiesto. Anche il **protocollo di misura** («metà tra ultima costa e cresta iliaca») è già scritto in UI, nel campo `alert` dell'indice.
> ⚠️ **AVVERTENZA — cosa succede se questa richiesta viene eseguita comunque:** implementarla oggi creerebbe un **secondo campo girovita**, con un proprio id, un proprio punto di salvataggio e una propria serie storica, **divergente dal primo** al primo referto in cui i due vengono compilati in modo diverso. È esattamente il difetto della famiglia F4 / P118 / P120 / F9 già pagato quattro volte in questo progetto: due fonti per lo stesso dato, e quella sbagliata che vince in silenzio in qualche schermata. Se serve altro sul girovita, si **estende** `ib-girovita` e l'indice `whtr` esistenti — non se ne aggiunge un altro.
> **Cosa resta davvero aperto di questo paragrafo:** solo l'**aggancio sindrome metabolica** (indice «X/5» in `CALCOLI_CLINICI`, v. sotto) e il resto del questionario di anamnesi. Il girovita e il WHtR **no**.

**RICHIESTA ESPLICITA DI FABRIZIO (30 lug 2026) — ⚠️ SUPERATA DAI FATTI, CONSERVATA COME STORIA (v. riquadro sopra):** il questionario deve includere la **misurazione del girovita in cm**. Non è una domanda ma una misura, e ha il miglior rapporto valore/costo della scheda: alimenta il **WHtR** (girovita/altezza, cutoff universale 0,5, predittore cardiometabolico migliore del BMI e valido senza correzioni per sesso ed etnia), è il criterio **obbligatorio** IDF per la sindrome metabolica (≥94 cm M / ≥80 cm F), e si muove prima e più del peso — spesso è l'unico segnale di miglioramento quando la bilancia è ferma. Nota tecnica: il rapporto Cintura/Fianchi che l'InBody 120 stampa è una **stima da BIA**, non una misura; il metro è un dato indipendente e più solido, quindi i due non vanno confusi nella stessa serie.
...
**SCHEDA:** Stato: Da fare (materiale da Fabrizio) — **ma il pezzo girovita/WHtR è già fatto e va escluso dallo scope** (verificato 4 ago 2026: `ib-girovita` + indice `whtr` cutoff 0,5 in `CALCOLI_CLINICI`) · Priorità: Bassa · C: 2 | I: 3 | R: 1 · Modello: Fable (Alto) per il mapping, Sonnet per l'implementazione · Autonomia: L2 sulle domande.
...
**FOCUS COMPONENTI COINVOLTI:** dati (REGOLE_SEMAFORO_ALIMENTI + FODMAP_PORZIONI), zero logica nuova.
**STATO DI PARTENZA MISURATO IL 10 AGOSTO 2026** (commit `a81fe0b`, incrociando `FODMAP_PORZIONI` col verbale del 28/7 — il documento di progetto è stato assorbito e chiuso quel giorno). **Prima la buona notizia, verificata voce per voce: nessuno dei valori sbagliati di Perplexity è entrato nel codice.** Sedano è rimasto tra i grigi senza porzione; i mirtilli non hanno il tetto superato di 40 g; la soglia inventata di 150 g sulle patate bianche non c'è; la «riduzione dei GOS del 30-40%» non c'è; il codice parla di banana *poco matura* (100 g) e di mais *in scatola* (75 g), non degli alimenti su cui Perplexity sbagliava. Le tre correzioni solide sono applicate: fragole 65 g, sedano fra i da evitare, patate dolci da 280 a 75 g.
**IL LAVORO CHE RESTA, QUANTIFICATO.** `FODMAP_PORZIONI` ha **16 voci, tutte con `fonte:'Monash'`**, ma quell'etichetta non distingue ciò che è stato verificato da ciò che è stato stimato: **4 corrispondono a una verifica tracciabile** (Ceci barattolo 42, Finocchio 75, Fragole 65, Patate dolci 75) · **3 sono elencati come NON VERIFICATI** nel verbale stesso (Yogurt greco 100, Zucca 75, Melone 120) · **8 non compaiono affatto** in quella verifica (Fagioli 40, Lenticchie 46, Fagiolini 75, Mais 75, Banana 100, Nocciole 15, Feta 40, Latte di soia) · **1 è in contraddizione col verbale**: il codice dice `Mandorle {max:20, nota:'~15 pezzi'}` mentre il verbale dice «10 mandorle = 12 g», cioè il codice è **~1,7× più permissivo** — da controllare per prima, perché è l'unico caso in cui due nostre carte si contraddicono. **Una sola voce su 16 porta la data** (Fragole, `Monash 2022`).
**DUE COSE DA SISTEMARE OLTRE AI VALORI.** (a) Il campo `fonte` **non viene stampato da nessuna parte**: esiste nei dati ed è invisibile, quindi il paziente riceve `• Yogurt greco 2% [max 100 g]` senza provenienza né data — il badge previsto qui sopra non è mai stato fatto. (b) ✅ **FATTA il 10 ago 2026** — il campo `stato` (`verificato` / `da-verificare`) è ora accanto a ogni voce di `FODMAP_PORZIONI`, con il commento che spiega cosa significa e come si promuove un valore. **Nessun numero clinico è stato cambiato**: i 16 `max` sono identici, verificati uno per uno prima della consegna. Suite 738/738 dopo il rigenero di INDEX.md. Da qui P130 non è più «da rifare tutta»: è una lista di **dodici righe**, e la prima da guardare è **Mandorle**, l'unico valore su cui due nostre carte si contraddicono (codice 20 g / ~15 pezzi contro il verbale del 28/7 «10 mandorle = 12 g», cioè ~1,7× più permissivo). Il commento nel codice lo dichiara accanto al dato. **Resta il punto (a)**: `stato` e `fonte` non sono letti da nessuno e non cambiano il PDF del paziente — mostrarli è una decisione tua sul documento che consegni, e si fa qui dentro. È la regola 20 applicata a questa tabella: *il nome della fonte accanto al dato non è l'identificatore che permette di ritrovarlo*.
**COME FALLISCE, DICHIARATO:** non è un allergene, è **permissività**. Un tetto troppo alto fa sì che il paziente rispetti l'indicazione e resti sintomatico, concludendo che la dieta non funziona — lo stesso danno che il vecchio PDF senza porzioni produceva. E i valori escono davvero dall'app: `_fodmapCostruisciPDF` → `inviaElencoFodmap`, con l'invio registrato in `p.invii[]`.
**SCHEDA:** Stato: Da fare · Priorità: Media (correttezza clinica su feature che si consegna al paziente) · C: 2 | I: 4 | R: 2 · Modello: fonte = Fabrizio con app Monash; inserimento = Sonnet · Autonomia: **L0 sui valori** (li detta Fabrizio dalla fonte).
...
**SCHEDA:** Stato: **Fase 0.5 ✅ CHIUSA · Fase 1 ✅ CHIUSA (cutover 1d, commit `97acb03`, collaudato in produzione il 24 lug 2026 — sync bidirezionale, tombstone, 4 meta-record OK) · Fase 2 AVVIATA (24 lug 2026: doppia scrittura `analisi_sangue`, prima entità — tabella popolata e verificata in collaudo).** 1d: i 4 meta-record vivono SOLO in `collections` (scrittura primaria via `_collectionsUpsert`, lettura unica via `_collectionsFetch`, tombstone P64 inclusi); helper doppia lettura `_preferNuovo`/`_tsMs` rimossi; righe finte in `pazienti` non più lette/scritte — backup + eliminazione manuale dopo collaudo. Fase 2: tabella `analisi_sangue` (SQL a Fabrizio) + `_analisiSangueUpsert` ombra best-effort nei 2 push del blob paziente, zero cambi di lettura. Al cutover serve reload forzato su PC e iPhone. Dettagli e collaudo richiesto nel CHANGELOG (voce 24 lug). Prossimi gradini fase 2 (ognuno L0): diff blob↔tabella → doppia lettura → estrazione dal blob con segnale versione minima; poi entità inbody → note; fase 3 = FK reali. Storia fasi precedenti ↓. **Fase 0.5 (16 lug 2026, commit `6ec732e`)** — variante **B2 pull differenziale**, dettagli nel CHANGELOG. **Fase 1, sotto-passi 1a+1b+1c:** 1a — tabella `collections` creata su Supabase (`{key,user_id,data jsonb,updated_at}`, RLS row-owner identica alle altre 5 tabelle, verificata da Fabrizio) (16 lug 2026, notte); 1b — doppia scrittura attiva (`_collectionsUpsert`) su tutti i 5 punti che scrivono i 4 meta-record, gated sul successo della scrittura legacy, **nessun cambio di lettura** (16 lug 2026, notte). **1c ✅ FATTA e collaudata in produzione (17 lug 2026, commit `e88e0fb`):** doppia lettura sui 4 read-point (`pullConcetiSupabase`, `_pazFetchMeta`, `pullAlimentiCustomSupabase`, `pullModelliSupabase`) — si legge SIA `collections` SIA il legacy e si sceglie il nuovo solo se presente, valido e **non più vecchio** del legacy (guardia `updated_at` via helper `_preferNuovo`/`_collectionsFetch`/`_tsMs`); in ogni altro caso ricaduta automatica sul legacy → **non può mai servire un dato più vecchio di prima**. Prima del 1c: verifica manuale di Fabrizio nel Table Editor che tutte e 4 le righe (`meta_collections`, `__concetti_educativi`, `__alimenti_custom`, `__modelli_rotazione`) si popolassero con l'uso reale e con lo stesso `user_id` (RLS ok). `_tsMs` normalizza il timestamp Postgres a prova di Safari/iPhone (spazio o `T`, `+00`/`+00:00`/`Z`, microsecondi). Verifica: `node --check` ok + 7 test unitari sulla logica di scelta + collaudo in produzione (PC e iPhone) di Fabrizio. **Prossimo passo:** collaudo del cutover 1d su PC e iPhone (reload forzato + spot-check + elimina-paziente↔sync) e verifica che `analisi_sangue` si popoli; poi backup + eliminazione delle 4 righe finte da `pazienti` e diff blob↔tabella per il gradino successivo della fase 2 · Priorità: Media-Alta · C: 5 | I: 5 | R: 4 · Modello: Fable 5 / Opus / Sonnet (sessione con più cambi modello, vedi CHANGELOG) · Autonomia: **L0**.
...
**COLLAUDO FASE 2: SUPERATO il 12 agosto 2026** (Fabrizio, sull'app in produzione, commit `ecdad4b`). Paziente reale, tema «Giorno gara», contesto compilato (orario 18:00, «12 km corsa», note «ricarica di carboidrati a colazione e a pranzo, cena leggera proteica»). Esiti: **1 solo giorno generato**, etichetta a tema conservata, giorno rimasto in coda al piano; **kcal 2758 contro un target di 2695 — +63 kcal, +2%**; **validatore clinico: «Nessuna violazione rilevata»** (203/248 elementi verificati). *I 45 «non verificabili» NON sono un difetto*: sono righe-ricetta senza grammature estraibili, che il validatore dichiara di non saper controllare invece di far finta — è la proprietà per cui è stato scritto così. Anche la **fase 1 dal Generatore a pillole** è confermata: i bottoni-toggle e il pannello contesto (P94b, `20b019d`) compaiono e funzionano.
**IL COLLAUDO HA PRODOTTO UNA VOCE NUOVA → P159**: il pulsante «Vai →» del pannello validatore sembrava non fare niente. Non era rotto — cambiava il giorno senza portarci lo sguardo. Dettagli nella sua scheda.
...
**RIFINITURE DOPO IL COLLAUDO (stesso giorno, commit successivo):** ordine dei riquadri scambiato fra composizione e adiposità (e reso dichiarativo in `_cOrdine`); numeri della card andamento riportati alla taglia delle altre tessere; **tetto di 52 settimane sulla proiezione all'obiettivo** (con peso quasi fermo usciva «circa 150 settimane», stessa famiglia del «−4,9 kg/sett»); etichette delle date sotto le barre del Ritmo uniformate alla sola data di fine.
...
### P131 — Adiposità centrale: scale arbitrarie e soglie senza fonte
**IL PROBLEMA:** il grafico sovrapponeva due assi verticali con estremi scelti a mano (0,85–0,93 e lv.3–lv.5): un calo di 0,7 livelli di viscerale sembrava un crollo, e il punto d'incrocio tra le due curve — che dipende solo da dove cadono gli estremi — sembrava significare qualcosa.
**LA SCOPERTA PIÙ IMPORTANTE (di Fabrizio):** le soglie del viscerale `<10 / 10–14 / ≥15` erano **in produzione senza nessuna fonte**, mentre il suo InBody dichiara **1–9**. Erano lì da prima di P99 e sono state ricopiate nei grafici nuovi senza chiedersi da dove venissero — regola 14 violata da chi l'aveva appena scritta.
**COSA È STATO FATTO (28 lug 2026):** Fabrizio ha scelto di **tenere un grafico solo** con i due assi (le proposte di separarlo in due pannelli o di spostare il viscerale in una striscia sono state scartate). Quindi: (a) soglie in `_IB_RIF` **con la fonte accanto al valore, e la fonte scritta a schermo** — viscerale 1–9 riferimento InBody, cintura/fianchi 0,80–0,90 uomo / 0,75–0,85 donna criterio OMS, entrambi i limiti mostrati; (b) **scale che si adattano al paziente** (`_ibFinestra`), perché una scala fissa 0–20 rende invisibile mezzo livello su un magro; (c) i due contrappesi che rendono onesto lo zoom: **righello clinico completo di fianco** con la finestra evidenziata, e **variazione scritta in numeri** a fine curva; (d) numeri degli assi colorati come le rispettive linee. Con questo la scheda InBody **non usa più Chart.js**.
**RESTA VERO:** due righelli restano due, quindi l'incrocio tra le curve continua a non significare niente. Se in visita capita di leggerci una relazione, è il segnale per separarle.
**SCHEDA:** Stato: **CHIUSA** ✅ 28 lug 2026 · Test 410/410 · Modello: Fable (Alto) · Autonomia: L1.

### P133 — Intervalli di riferimento dal referto InBody + grafico a tre asticelle
**DA DOVE NASCE:** Fabrizio usa in visita il grafico InBody "Analisi Muscolo-Grasso" (tre asticelle peso / muscolo / grasso con zone Sotto-Normale-Sopra) e ci spiega le forme: **a C** (peso e grasso alti, muscolo basso), **a D** (poco peso, tanto muscolo, poco grasso), oppure le tre asticelle **equivalenti**. Vuole qualcosa di simile in NutriGest.
**PERCHÉ NON SI PUÒ COPIARE, E NON È (SOLO) UNA QUESTIONE DI DIRITTI:** l'idea e la lettura clinica non sono proteggibili, la loro grafica sì — quindi una versione nostra si può fare. Ma **le asticelle InBody sono in percentuale rispetto a un valore "standard" calcolato da una formula loro che non abbiamo**: ricostruire quella scala vorrebbe dire inventarsi il denominatore, cioè rifare l'errore del livello 15.
**LA SOLUZIONE:** il referto **stampa già l'intervallo di riferimento** accanto a ogni misura (`Peso 109,8 ( 52,8~71,4 )`). NutriGest ha già questa disciplina per il sangue ("sul referto del paziente fa sempre fede il range del laboratorio"): va estesa all'InBody. **Tappa 1** — il prompt di estrazione (`loadInbodyPDF`) cattura gli intervalli e li salva accanto ai valori. **Tappa 2** — il grafico a tre asticelle con le fasce di riferimento **allineate in verticale** (è ciò che fa emergere la forma) e il **riconoscimento automatico della forma** con la frase che Fabrizio direbbe a voce: è la parte che lo rende nostro e non loro. Prototipo grafico già fatto e approvato.
**COSA È STATO FATTO (28 lug 2026):** entrambe le tappe. **Tappa 1** — il prompt di `loadInbodyPDF` chiede anche gli intervalli stampati tra parentesi (l'esempio nel prompt viene da un referto reale InBody120); `_ibPuliRiferimenti` scarta un intervallo malformato invece di aggiustarlo; gli intervalli viaggiano in `window._ibRifImport` (azzerato da `openInbody`) e finiscono in `ib.rif`. **Tappa 2** — `_ibGrForme`: fasce di riferimento allineate in verticale e **riconoscimento automatico della forma** con la frase clinica (`_ibFormaDi`). Il grafico non compare se il referto non stampa gli intervalli.
**SCHEDA:** Stato: **CHIUSA** ✅ collaudata da Fabrizio il 29 lug 2026 sul primo import reale: gli intervalli arrivano dal referto, la card "Peso · Muscolo · Grasso" compare e la forma (a C) viene riconosciuta. · Test 410/410.

### P134 — Scheda InBody: filtri di periodo e lettura dei valori su telefono
**IL PROBLEMA:** Fabrizio ha pazienti da 4 e da 25 misurazioni, e cresceranno. Con 25 referti Ritmo e Mappa della qualità diventano illeggibili (già mitigati con lo scorrimento laterale, 28 lug). E **su iPhone i valori dei grafici non si vedono affatto**: su PC li dà il passaggio del mouse, sul telefono non c'è equivalente.
**LA SOLUZIONE:** (a) riga di pulsanti **6 mesi · 1 anno · 2 anni · Tutto** sopra la scheda, che filtra tutti i grafici insieme, default "Tutto", con scritto a schermo quante misurazioni si stanno vedendo su quante — mai troncare in silenzio; (b) **tocco / pressione prolungata per leggere i valori** su tutti i grafici, equivalente mobile del passaggio del mouse. Scartato invece il "tenere premuto per far comparire periodi nascosti": è un gesto che non si scopre e su PC non esiste.
**COSA È STATO FATTO (29 lug 2026):** implementate entrambe le parti; **la parte (a) è stata poi RIMOSSA su richiesta di Fabrizio** (vedi sotto). Resta in vigore solo il tocco.
**(a) Il filtro.** `_ibFiltraPeriodo` taglia a partire dall'**ultima misurazione**, non da oggi: chi non viene da un anno vedrebbe altrimenti "6 mesi" vuoto. In `renderPdInbody` l'archivio completo (`sortedTutte`) e la finestra scelta (`sorted`) sono due variabili distinte: i grafici usano la finestra, **la tabella in fondo e i conteggi usano sempre l'archivio**, così l'etichetta "Tutte le misurazioni (N)" non diventa una bugia. La barra compare solo da 4 misurazioni in su; un pulsante che lascerebbe meno di 2 misurazioni è **disattivato** e dice perché; cambiando paziente si riparte da "Tutto". Sotto i pulsanti c'è sempre scritto quante se ne vedono su quante, con la frase "le altre restano a un clic, non sono cancellate": il filtro non deve mai far pensare a una perdita di dati.
**(b) Il tocco.** Un `pointerdown` che ignora `pointerType==='mouse'` (il mouse continua ad avere il passaggio) e riusa **la stessa etichetta e gli stessi bersagli invisibili** del desktop — nessun testo scritto due volte, nessuna soglia di pressione da indovinare. L'etichetta esce **sopra il dito**, che altrimenti coprirebbe ciò che si è appena toccati, e si chiude toccando fuori, allo scorrimento o dopo 5 secondi.
**TRE COLLISIONI DI TESTO TROVATE GUARDANDO IL RENDER A 430px** (nessun test le vedeva): didascalia della mappa qualità sopra il titolo dell'asse → va a capo; "NELLA NORMA" più larga della sua fascia → "NORMA" quando è stretto; fonti dell'adiposità centrale tagliate dal bordo → due righe.
**LA PARTE (a) È STATA TOLTA (29 lug 2026, stessa giornata).** Fabrizio, vedendola in uso: *"non mi piace questa modifica che hai fatto con i 3 filtri, voglio tornare come era prima"*. La riga di pulsanti, il CSS e la separazione archivio/finestra sono stati rimossi: la scheda torna a mostrare sempre tutte le misurazioni. **Da ricordare se l'idea dovesse tornare:** il problema di partenza (25 referti illeggibili) resta risolto dallo scorrimento laterale delle card, non dal filtro; e la ragione per cui il filtro dava fastidio è che si aggiungeva un comando in cima alla scheda per un problema che in visita non si sente. Non è un fallimento tecnico: è che la scheda si guarda tutta intera.
**SCHEDA:** Stato: **CHIUSA** ✅ 29 lug 2026 — solo la parte (b), il tocco · Test 410/410 · Modello: Sonnet (Media) · Autonomia: L1.

### P138 — Scheda InBody: pulizia estetica e armonia fra i riquadri (CHIUSA)
**DA DOVE NASCE:** Fabrizio, dopo P137: "ci sono altre cose che voglio migliorare per rendere la sezione InBody più pulita esteticamente… **regola di base è che ci deve essere armonia di grandezza tra i grafici**". Otto richieste puntuali, tutte accolte.
**COSA È STATO FATTO (29 lug 2026):**
- **Peso casalingo e Memoria paziente affiancati in cima** (una colonna sotto i 900px). Prendevano una riga intera ciascuno: due fasce larghe quanto lo schermo prima ancora del primo grafico. Nella Memoria i delta scendono a una riga di testo piccolo — lì quello che serve è il pulsante dell'ascolto.
- **Via la fascia verde "Dal … ad oggi"**: gli stessi tre numeri stanno già nelle tessere di Composizione nel tempo.
- **Via SETTE didascalie** sotto i grafici (tempo, barre, ritmo, qualità, adiposità, peso·muscolo·grasso) e la riga "Media reale…" dal piede del Ritmo. La data di partenza di Composizione nel tempo passa nella testata del riquadro (`.ib-hd-sub`), come la fonte delle fasce in Peso·Muscolo·Grasso.
- **Analisi segmentale a metà riga** (non più a riga intera) con accanto la tendina "Tutte le misurazioni". La tendina resta a metà anche da aperta — scelta di Fabrizio — e scorre di lato; si allarga solo quando i grafici sono in numero pari e resterebbe da sola.
- **Armonia:** le altezze dei grafici stavano fra 234 e 360 px, ora tutti sui 300 (tempo 262→300, qualità max 360→320, barre 340→320, ritmo 306→290 senza il piede, forme 234→286). `.ib-g2col` passa a `align-items:stretch`: due riquadri accanto finiscono alla stessa altezza.
**DUE COSE SALVATE, PERCHÉ NON ERANO DIDASCALIE MA INFORMAZIONE:** nella riga tolta dal Ritmo c'era **`muscolo −0,01 kg/sett`**, che non stava scritto da nessun'altra parte → è salito in testata accanto a "Media grasso". Il **tratteggio** delle barre (periodo sotto le 3 settimane) non aveva più chi lo spiegasse → è diventato un quadratino nella legenda in alto. E la **fonte delle fasce** di Peso·Muscolo·Grasso (regola 14) è passata nella testata invece di sparire.
**COSA SI PERDE, DICHIARATO:** nella Mappa della qualità i pallini restano numerati ma l'elenco `1 22 ago→3 set · …` non c'è più: la data del periodo si legge solo col passaggio del mouse o col tocco. Fabrizio ha scelto così sapendolo.
**SCHEDA:** Stato: **CHIUSA** ✅ 29 lug 2026 · Test 410/410 · Modello: Opus (Alto) · Autonomia: L1.

### P137 — Grafici InBody alla prova dei 25 referti veri (CHIUSA)
**DA DOVE NASCE:** primo collaudo su una paziente reale con 25 misurazioni, periodi da 12 giorni ad anni, un dato anomalo (+24 kg di muscolo in una misurazione) e ±35 kg di escursione. Verdetto di Fabrizio, grafico per grafico: Composizione nel tempo e Adiposità centrale schiacciate ("voglio la barra di trascinamento, così ogni misurazione ha il suo spazio"), Ritmo "orribile", Mappa della qualità bella ma con i numeri degli assi "orribili". Composizione pezzo per pezzo: da non toccare.
**COSA È STATO FATTO (29 lug 2026):**
- **`_ibPasso`** — passo degli assi "gentile" (1·2·5 per potenza di dieci, ~5–8 etichette sempre): prima il passo era fisso per grafico e con ±35 kg uscivano oltre trenta etichette accatastate. Applicato a Composizione nel tempo, Ritmo, Mappa della qualità (entrambi gli assi) e Adiposità.
- **Scorrimento laterale anche per Composizione nel tempo e Adiposità centrale** (come già Ritmo e barre), solo oltre le 8 misurazioni: sotto, il layout resta identico a oggi.
- **Ritmo, scala robusta:** la scala segue il grosso dei dati (85° percentile); un solo periodo anomalo da ±3 kg/sett non schiaccia più tutte le altre barre in una striscia. Le barre tagliate hanno il marcatore ▲/▼ e il conteggio dei fuori scala è SCRITTO nel grafico — si taglia, mai in silenzio. Sotto i 30px la scritta "12 gg" non si stampa (resta la lineetta rossa e il valore nel tocco).
- **Date scelte sui pixel** (`_ibDate`): con l'asse del tempo reale, "una data ogni N misurazioni" faceva fondere le date dei referti ravvicinati ("22 ago" sopra "24 set").
- **Etichette di fine curva che si scansano** se le curve convergono (tempo e adiposità): il trattino resta agganciato al punto vero, così lo spostamento è dichiarato.
**SCHEDA:** Stato: **CHIUSA** ✅ 29 lug 2026 · Test 410/410 · Modello: Fable (Alto) · Autonomia: L1.

### P136 — Grafici InBody enormi finché non si toccava lo zoom (CHIUSA)
**IL SINTOMO (visto da Fabrizio):** aprendo la scheda InBody i grafici uscivano sgranati e con i testi giganti; bastava cambiare lo zoom della pagina da 100 a 90 e tornare a 100 perché si vedessero bene.
**LA CAUSA:** `renderPdInbody` viene eseguita da `openPaz` **mentre la linguetta InBody è ancora nascosta**. Lì `clientWidth` vale 0, e `_ibDisegnaSvg` cadeva sul valore di ripiego (240–320 px): il grafico veniva disegnato stretto e poi **stirato dal CSS** (`width:100%`) fino alla larghezza vera. Misurato sulla versione pubblicata: fattore di stiramento **2,02×** su tre riquadri su cinque. L'unico evento che faceva rientrare il codice era `resize` — ecco perché il cambio di zoom "riparava".
**LA CORREZIONE (29 lug 2026):** (a) se il riquadro misura meno di 80 px **non si disegna affatto** — una larghezza inventata è peggio di un riquadro momentaneamente vuoto; (b) un `ResizeObserver` su `#pd-inbody` ridisegna appena i riquadri ricevono la larghezza vera (passaggio da nascosto a visibile, cambio zoom, cambio finestra); (c) `data-ibw` memorizza la larghezza già usata, così l'osservatore non innesca ridisegni a vuoto.
**LA LEZIONE:** il difetto è vissuto per settimane perché **in collaudo la scheda veniva sempre resa visibile prima di disegnarla** — la prova non riproduceva il percorso reale dell'utente. Il test ora costruisce la scheda da nascosta, la mostra, e verifica che il rapporto larghezza-riquadro / larghezza-disegno sia 1,00.
**SCHEDA:** Stato: **CHIUSA** ✅ 29 lug 2026 · Test 410/410 · Modello: Sonnet (Media) · Autonomia: L1.

### P135 — Scheda InBody: lista misurazioni e ingrandimento (CHIUSA)
**IL PROBLEMA:** in fondo alla scheda c'era una card per ogni misurazione — nove numeri ciascuna, già raccontati dai grafici sopra. Con 7 referti ~1200px, con 25 una pagina infinita. E un difetto meno ovvio: **le card non si possono confrontare**, per seguire una singola voce (l'acqua, il metabolismo) bisogna saltare da un riquadro all'altro.
**COSA È STATO FATTO (28 lug 2026):** tabella dentro un `<details>` chiuso, con decimali fissi per colonna — **si legge in colonna**, che è la cosa che le card non potevano fare. In più, pulsante ⤢ su ogni riquadro che lo ingrandisce a tutto schermo: overlay proprio e non Fullscreen API, perché su iPhone Safari il fullscreen funziona solo sui video; il grafico viene **ridisegnato** alla larghezza grande, non ingrandito come immagine. Analisi segmentale a riga intera.
...
**➔ ASSORBITA IN P153 (6 ago 2026):** non si lavora separatamente. L'intuizione del «falso amico» e la prescrizione `misurazione.fonte` restano valide e sono confluite nel modello dati unico di P153; il lavoro concreto è la **tappa 153b**.

### P153 — Le altre bioimpedenziometrie: aprire l'app a chi non ha né InBody né plicometro (aperta 6 ago 2026, nessun codice scritto)
**DA DOVE NASCE:** Fabrizio, 6 agosto 2026: *"oltre la bioimpedenziometria di InBody e la plicometria vorrei aggiungere in NutriGest la possibilità per tutti i nutrizionisti di usarlo anche se non hanno la BIA di InBody o la plicometria."* Stesso movente commerciale di P139: **oggi NutriGest ha un presupposto nascosto — serve un InBody** (il motore TDEE parte dal MB sulla massa magra InBody, il traguardo P122 dalla massa magra InBody, la scheda composizione è una scheda InBody).
**LA DOMANDA GIUSTA NON È «QUANTI DISPOSITIVI», È «QUANTE FORME DI DATO».** L'istinto è implementare una marca alla volta: ogni marca un modulo, ogni modello nuovo sul mercato un'altra voce di roadmap — una coda che non finisce mai. Il censimento dice altro: **i dispositivi sono decine, le forme del dato sono due.** È lo stesso ragionamento di P124: non abbiamo scritto un lettore per ogni laboratorio italiano, abbiamo scritto un lettore di *tabelle di referto*, e il numero dei laboratori è diventato irrilevante.
**FAMIGLIA A — consegnano l'elettricità.** Rz, Xc e angolo di fase, quasi sempre a 50 kHz, quattro elettrodi adesivi, paziente sdraiato; la composizione corporea non la misurano, la **calcolano** con equazioni di regressione scelte dal clinico. **Akern** (BIA 101 BIVA PRO / Bodygram — azienda italiana, il nome che ricorre di più negli studi italiani), Bodystat, Maltron, RJL/BIA 310e, BioTekna BIA-ACC. Il capostipite metodologico è la **BIVA di Piccoli**: il vettore Rz/Xc normalizzato per l'altezza, letto su un'ellisse di riferimento, *senza* passare per i compartimenti — ed è il motivo per cui la famiglia esiste ancora: su un paziente edematoso o disidratato le equazioni sbagliano, il vettore no.
**FAMIGLIA B — consegnano i compartimenti.** Multifrequenza segmentali, pedana con impugnature, equazioni proprietarie **non pubblicate**: esce già massa grassa, magra, muscolo scheletrico, acqua totale ed extracellulare, spesso per distretto. **InBody** (fatto), **Tanita** (MC-780MA/U, MC-580, DC-360), **seca mBCA** (515/525/554), Jawon, Charder, Evolt; in fondo alla scala le bilance domestiche dei pazienti (Withings, Omron, Garmin, Tanita BC-545N).
**LA PRIORITÀ È CONTROINTUITIVA, E VA DETTA.** L'istinto dice «facciamo Akern, è lo strumento italiano». Ma Akern è la famiglia col **motore da scrivere**; la famiglia B si apre quasi gratis riusando lo stampo InBody. Se il fine dichiarato è *rendere l'app usabile da chiunque*, si parte da dove il rapporto fra nutrizionisti sbloccati e codice scritto è più alto, non da dove il nome è più prestigioso.
**QUELLO CHE NON SI FA MAI — convertire fra strumenti.** Alla domanda «il paziente prima faceva l'InBody, ora ho un Tanita: converti lo storico?» la risposta è **no**, e il rifiuto va scritto nel codice, non lasciato al buon senso. Le equazioni della famiglia B sono proprietarie; e anche restando dentro la famiglia A, due equazioni diverse sulla stessa coppia Rz/Xc danno massa magra diversa. Un valore convertito sarebbe **un numero inventato con l'aria di un numero misurato** — regola 11, e P124 (*un suggerimento sbagliato è peggio di nessun suggerimento*). La risposta giusta è: **si cambia serie**, come le due bilance di P35 e come P101 prescriveva già.
**TRE LIVELLI D'INGRESSO, IN ORDINE DI COSTO.** (1) **A mano**: una scheda «misura» con lo strumento scelto da un elenco e i campi canonici tutti facoltativi — copre il 100% dei dispositivi esistenti **e futuri**, il giorno stesso in cui escono. (2) **Export CSV/XLS** del software del dispositivo (Bodygram, gestionale Tanita, seca analytics, LookinBody): mappatura colonne→campi per produttore, col tranello già noto delle intestazioni localizzate e della **virgola decimale italiana** — su dato di fonte esterna la virgola si scarta, non si converte (P128). (3) **Referto stampato letto dall'AI**: qui **P124 è riusabile quasi per intero** — orientamento rilevato guardando la pagina invece di credere al metadato `/Rotate`, una pagina per chiamata, impronta della riga, data del test che non può coincidere con la data di nascita, casella correggibile a mano prima di ogni controllo automatico.
**IL TRANELLO SPECIFICO DEL REFERTO BIA — i grafici a barre.** Il foglio InBody o Tanita mostra numeri *e* barre: l'AI che guarda l'immagine può leggere la posizione della barra invece della cifra. Lo smaschera la **coerenza interna dei compartimenti** (grasso + magra = peso; acqua totale dentro la massa magra; ECW/TBW in intervallo fisiologico): tre identità che *devono* tornare. È la regola 18 — *se i dati sono legati da identità aritmetiche, il controllo non ha bisogno né dell'AI né del documento originale* — applicata a dispositivi diversi da quello di P63b.
**IL MODELLO DATI:** una sola entità `misura`, indipendente dal dispositivo — `strumento` (marca, modello e **metodo**: BIA-monofrequenza / BIA-multifrequenza / BIA-segmentale / plicometria / DEXA / scanner-3D); campi **grezzi** separati (`Rz`, `Xc`, `frequenza`, `angoloFase`); campi **derivati** canonici, tutti annullabili; e per ogni derivato l'`origine`: **`letto`** dal referto oppure **`calcolato`** da noi, con l'`equazione` dichiarata accanto. I due non si mescolano mai nella stessa serie. Vale parola per parola la regola di P128: **`null` non è zero e non è «va bene»** — un dispositivo che non misura l'acqua extracellulare lascia il campo vuoto, l'app non ci scrive 0.
**NEI GRAFICI:** una serie per strumento, cambio di strumento marcato sull'asse del tempo, nessuna media fra serie diverse. E la soglia `_IB_MIN_GG=21` **non si copia**: è tarata sulla BIA InBody, ha senso su una BIA e molto meno su una plicometria.
**RAPPORTO CON P101 (Referti non-InBody):** P101 aveva già visto il rischio giusto — il «falso amico» — e prescritto `misurazione.fonte` più la segnalazione del cambio fonte nei grafici invece della fusione silenziosa. **P153 la contiene e la generalizza:** P101 restava dentro l'impianto InBody (un prompt più generico), P153 sposta il problema sul modello dati. **P101 si chiude dentro la tappa 153b e non si lavora separatamente.**
**RAPPORTO CON P139 (Plicometria) — e una decisione di ordine da prendere.** Anche la plicometria è «uno strumento che produce una misura di composizione corporea con la sua equazione». Se **153a viene fatta prima di P139**, la plicometria entra come *caso* del modello generale invece che come secondo motore separato, e il progetto si ritrova con **un'entità sola invece di tre** (InBody, pliche, altre BIA). È probabilmente la mossa che fa risparmiare più lavoro complessivo — ma rimanda una voce già progettata nel dettaglio. **Decisione di prodotto, da prendere consapevolmente, non per inerzia.**
**IL DATO CHE MANCA, DICHIARATO COME MANCANTE.** Che i dispositivi esistano e cosa producano è verificato sulle schede tecniche dei produttori. **Quanto siano diffusi negli studi italiani NON è verificato e non è pubblicato da nessuno:** le ricerche di mercato danno il giro d'affari globale e i tre nomi in testa (InBody, Tanita, seca), non il parco installato italiano; i siti dei singoli professionisti sono un campione autoselezionato (chi ha speso per un Akern lo scrive in homepage, chi ha una Tanita da 400 € no). **La fonte migliore non è una ricerca di mercato: è chiedere a dieci nutrizionisti che strumento hanno.** Finché quel dato manca, l'ordine fra 153b e 153c non è decidibile su basi solide — regola 16: *contare non è ancora il lavoro, il lavoro è classificare.*
**TAPPE:** **153a** modello dati `misura` + inserimento a mano + una serie per strumento nei grafici — costo medio, sblocca **tutti** i dispositivi subito · **153b** referto PDF/foto della famiglia B riusando P124 + i tre controlli di coerenza dei compartimenti (**assorbe P101**) — costo medio-basso, il grosso è già scritto · **153c** motore famiglia A, da Rz/Xc ai compartimenti, con equazione dichiarata e scelta dal clinico — **costo alto: è ricerca clinica prima che codice** (Sun 2003, Kyle 2001; le fonti primarie si cercano su PubMed, connettore collegato il 6 ago 2026) · **153d** vettore BIVA (ellisse di Piccoli) come lettura a sé, senza compartimenti · **153e** import CSV dagli export dei software dei dispositivi — basso per formato, ma uno per formato.
**RAGIONAMENTO COMPLETO:** `claude/NutriGest_P153_Altre_Bioimpedenziometrie_Ragionamento.md` nel progetto Claude — censimento per famiglia con le fonti dei produttori, le tre domande ancora aperte e il confronto coi software concorrenti italiani.
**SCHEDA:** Stato: **Da fare** — progettata, nessun codice · Priorità: **Media-Alta pre-vendita** (insieme a P139 è ciò che toglie il presupposto InBody) · C: 4 | I: 5 | R: 3 (ennesima fonte di composizione corporea) · Dipendenze: P124 per 153b; nessuna per 153a · Modello: Fable/Opus (Alto) per 153c-d, Sonnet per 153a/b/e · Autonomia: **L0**.

### P139 — Plicometria: il secondo motore d'ingresso (aperta 30 lug 2026, nessun codice scritto)
**DA DOVE NASCE:** Fabrizio: *"molti nutrizionisti usano la plicometria come metodo di misura, vorrei aggiungerla. Ma prima ragioniamo se ne vale la pena."* **Fabrizio non ha mai usato il plicometro**: la voce non nasce da un bisogno della sua pratica ma dal posizionamento commerciale.
**IL VERDETTO — sì, ma il motivo giusto è uno solo.** Come *stima della % di grasso* la plicometria a lui non serve: ha l'InBody, otterrebbe un secondo numero meno preciso della stessa grandezza. Il valore vero è che **oggi NutriGest ha un presupposto nascosto: serve un InBody.** Il motore TDEE parte dal MB sulla massa magra InBody, P122 deriva il traguardo dalla massa magra InBody, la scheda composizione è una scheda InBody. Un collega senza InBody compra l'app e ne trova spenta la metà caratterizzante. Mercato verificato: Dietosystem la vende come modulo dedicato, WinFood/MetaDieta/Bodygeo/Sifa la hanno; i gestionali economici (Nutribook, Appuntoo, Nutriverso) non gestiscono composizione corporea. **Non averla colloca NutriGest in quella seconda fascia.**
**LA PROVA NUMERICA CHE DECIDE L'ARCHITETTURA (calcolata, uomo 42 anni 88 kg 178 cm, pliche plausibili):** Jackson-Pollock 3 → **18,6%**, JP7 → 20,0%, Durnin-Womersley → 27,3%, Peterson → 27,2%. **8,8 punti di ampiezza sullo stesso paziente**, che propagati a Katch-McArdle fanno **da 1.751 a 1.918 kcal di MB — 167 kcal decise da quale riga di codice si è scelta.** Dal lato opposto, il minimo cambiamento *rilevabile* (MDC₉₅ = 2,77 × TEM) su una Σ7 è ≈ 7 mm, cioè **meno di 1 punto percentuale**. L'errore del metodo è dieci volte il segnale.
**LA DECISIONE ARCHITETTURALE:** **si salvano i millimetri, la percentuale è derivata.** Indicatore primario = **somma pliche in mm** e sua traiettoria; la % grasso è output terziario collassato, con equazione ed errore dichiarati accanto. È la terza applicazione del pattern "fonte di verità grezza + specchio derivato" già usato in P118 tappa 1 e in P122 (`pesoTarget`). Posizione convergente della letteratura recente (German J Sports Med 2022; position statement IOC/Ackland 2012; prassi ISAK): convertire in percentuale aggiunge un passaggio d'errore più grande del segnale.
**IL RISCHIO PRINCIPALE — è il "falso amico" di P101, auto-inflitto.** Diventerebbe la terza fonte di verità sulla composizione corporea (InBody, peso casalingo, pliche). Famiglia già vissuta due volte (P118, P120): è la ragione delle lezioni 10-11 del CLAUDE.md. **Serie plicometrica SEMPRE separata da quella InBody, mai fuse in un unico grafico o campo "% grasso".** L'unico confronto legittimo fra i due metodi è **indicizzato alla variazione dal basale** — cioè il grafico già scelto in P99 (Composizione nel tempo, proposta A).
**SCELTE TECNICHE GIÀ FATTE:** schema dati **ISAK a 8 pliche** (standard formativo in Italia, sovrainsieme delle equazioni classiche) + profilo ACSM selezionabile per JP7. Equazione di default **Durnin-Womersley** (l'unica citata da Società Italiana di Endocrinologia e ISS/EpiCentro → difendibile in Italia; bias −2% vs 4C contro −6,6% di JP). Siri/Brozek selezionabile e sempre dichiarata. Equazione **congelata sul paziente**, come il plicometro e il lato. **Faulkner NON come default:** non è di Faulkner, è un errore bibliografico smentito nel 2007 (deriva da coefficienti mai pubblicati di Yuhasz) e non ha popolazione di validazione né SEE documentati; se inclusa, va etichettata come storica non validata.
**TRE DETTAGLI CHE DECIDONO SE LA FUNZIONE VIENE USATA O ABBANDONATA:** (a) inserimento **per SERIE, non per sito** — ISAK prescrive il giro completo degli 8 siti poi si ricomincia, perché il tessuto compresso non torna subito allo spessore e la seconda misura di fila è sistematicamente più bassa; una griglia sito×ripetizione incoraggia l'errore; (b) **dettatura vocale** — durante la misura si hanno due mani sul calibro e le dita non lasciano la presa, senza voce si torna al foglio di carta; (c) **guida al reperto sempre a schermo**, non in una schermata di aiuto: 1 cm di spostamento cambia il valore nel ~70% dei siti.
**VINCOLO NORMATIVO VERIFICATO:** linee guida ONB/FNOB (delibera 433/2019, par. 7) vietano di demandare il rilievo al cliente → **niente inserimento pliche lato app paziente**, mai. Il biologo nutrizionista può invece eseguirla senza vincoli (par. 8, apparecchi non invasivi). Restare su "calcolo e registro" invece che su classificazioni diagnostiche automatiche tiene l'app lontana dalla soglia MDR (Reg. UE 2017/745).
**IL RISCHIO DI COLLAUDO, DICHIARATO:** Fabrizio non sa usare il plicometro, quindi **non può collaudare sul campo** — e la lezione del 29 lug dice che su questo genere di lavoro il collaudo vero è guardare il risultato reale. Contromisure, in ordine di costo: (1) collaudo **numerico** delle equazioni nella test-suite contro calcolatori indipendenti (casi di prova già calcolati nel documento); (2) un plicometro da €30 per provare l'interfaccia con due mani occupate; (3) **un collega che la usa già** — vale più di tutta la bibliografia, da cercare prima della tappa 3.
**TAPPE:** **1** registro plicometrico (`p.pliche[]` + `p.plicoSetup`, inserimento per serie, terza misura automatica sopra il 5% di scarto, dettatura, fuori scala) → somma in mm, **nessuna % grasso, nessun aggancio al TDEE** (4-5 h) · **2** grafico con **banda di errore** + mappa del corpo + marker di discontinuità su cambio operatore/strumento (3-4 h) · *— qui ci si ferma e si collauda —* · **3** motore delle equazioni (3 h) · **4** aggancio: `fonteComposizione`, FFM → Katch-McArdle → TDEE con **MB mostrato come intervallo**, `metodoMisura` sul traguardo P122 (3 h) · **5** circonferenze e indici, vita/WHR/WHtR/MUAMC/AMA (2-3 h).
**PERCHÉ QUEST'ORDINE:** le tappe 1-2 sono deterministiche e verificabili a tavolino; le 3-5 richiedono giudizio clinico. L'ordine fa cadere il rischio di collaudo il più tardi possibile. E le tappe 1-2 da sole sono già un prodotto: un registro plicometrico con banda d'errore è più di quanto documenti pubblicamente qualsiasi concorrente italiano.
**RAGIONAMENTO COMPLETO:** `claude/NutriGest_Plicometria_Ragionamento.md` nel progetto Claude — contiene le formule esatte di tutte le equazioni con i coefficienti completi, la tabella Durnin-Womersley per fascia d'età, i sei punti bibliografici ancora da chiudere prima di codificare (§11.1), il tranello anatomico delle tre "sovrailiache" e le fonti primarie.
**NOTA DEL 6 AGOSTO 2026 — leggere P153 prima di aprire questa voce.** P153 (le altre bioimpedenziometrie) definisce un modello dati `misura` indipendente dal dispositivo. Se la tappa **153a** viene fatta per prima, la plicometria entra come *caso* di quel modello invece che come motore separato: si evita di costruire la terza struttura dati sulla composizione corporea. L'ordine fra le due è una decisione aperta, esplicitamente non ancora presa.
**SCHEDA:** Stato: **Da fare** — progettata, nessun codice · Priorità: **Media-Alta pre-vendita** (è ciò che rende l'app usabile senza InBody) · C: 4 | I: 5 | R: 3 (terza fonte di composizione corporea) · Modello: Fable/Opus (Alto) per le tappe 3-4, Sonnet per la 1-2 · Autonomia: **L0**.
...
### P40 — Passi giornalieri + storico ⚠️ **PARZIALE — il campo c'è ed è già in uso clinico, manca lo storico**
**⚠️ ATTENZIONE PRIMA DI TOCCARLA (verificato 4 ago 2026): il campo passi NON è da fare, esiste e decide già le calorie del paziente.** Questa voce diceva «campo passi + grafico al trigger smartwatch» come se fossero entrambi da costruire. In realtà:
- **Il campo esiste** in anagrafica: `grep -n "passiGiornalieri" index.html` → input `p-passi` letto e scritto in tutti i punti del ciclo di vita del paziente (`p.passiGiornalieri=gn('p-passi')` in `salvaPaz`, precompilazione del modal, riga «Passi/giorno» nella scheda, preservazione fra i campi anagrafici).
- **E soprattutto alimenta già il motore TDEE:** `grep -n "function calcolaTDEE" index.html` → dentro `calcolaTDEE` la riga `const passi = p && p.passiGiornalieri!=null ? +p.passiGiornalieri : null;` entra in `hasDatiAttivita` e nella **curva NEAT** del metodo MET additivo (v. anche `_neatFrazione` e le righe «→ NEAT … kcal» del contesto AI). Conseguenza pratica: **cambiare i passi cambia il TDEE e quindi le kcal prescritte**. Se `passiGiornalieri` manca, `calcolaTDEE` ripiega sul selettore LAF manuale — è il ripiego di cui parla F8.
- **Da non fraintendere:** la frase «non entrano in nessun calcolo» che compare accanto ai passi in una riga del contesto AI riguarda **tipo/intensità/ore di allenamento** (voce P126), **non** i passi.

**QUELLO CHE MANCA DAVVERO — ed è tutto il residuo di questa voce:** i passi sono **un solo numero dichiarato**, non una serie. Non esiste **storico dei passi nel tempo** né **grafico/sparkline**: non si può vedere se il paziente si sta muovendo di più o di meno da una visita all'altra, e il numero dichiarato a gennaio continua a calcolare le calorie ad agosto senza che nulla dica che è vecchio.
**Critica (invariata):** non creare l'ennesima serie storica ad hoc: lo storico nasce già nella forma misurazioni-per-data (come `pesiIntermedi`), sparkline riusata da P35. **Nota aggiunta il 4 ago 2026:** quando si farà, va deciso esplicitamente **quale valore alimenta `calcolaTDEE`** — l'ultima misurazione, una media, o il campo dichiarato che resta com'è. Aggiungere una serie senza rispondere a questa domanda crea due fonti per lo stesso numero (famiglia F4/F9) su un dato che **muove le calorie**.
**Componenti:** dati+frontend. **SCHEDA:** Stato: ⚠️ **PARZIALE — campo passi in produzione e attivo nel calcolo TDEE; storico e grafico DA FARE** (trigger smartwatch) · Priorità: Bassa · C1|I2|R1 · Sonnet (Bassa) per lo storico/grafico, **ma L0/decisione di Fabrizio** sulla domanda «quale valore entra nel TDEE» · L1.
...
### P47 — Keto avanzato
**Sintesi:** template dai piani keto storici + reintroduzione carbo a step. **Critica:** dipende da P48 (i piani vanno prima digitalizzati) — la roadmap non lo dice; ~~la reintroduzione a step è un PRESET SEQUENZIALE~~ superata: Fabrizio ha chiesto gestione MANUALE caso per caso, non preset sequenziali. **Componenti:** contenuti + P3. **SCHEDA:** METÀ FATTA (21 lug 2026, commit P-KETO-USCITA — vedi CHANGELOG): reintroduzione carbo implementata come modale manuale "Uscita graduale" + blocco REINTRODUZIONE nel prompt generatore + regola frutta/legumi keto. RESTA da fare: template dai piani keto storici (dipende da P48) · Bassa · C3|I3|R2 · Fable (Alto) · L2.
...
**Tappa 2 — Range di riferimento ✅ CHIUSA E VALIDATA 24 lug 2026.** `RANGE_STD` copre tutte e 119 le voci; il range stampato dal laboratorio viene estratto all'import e ha la precedenza; la riga compare sotto ogni valore. Tabella **validata da Fabrizio il 24 lug 2026**: nei referti reali il laboratorio stampa quasi sempre il proprio intervallo, che ha la precedenza, quindi RANGE_STD entra in gioco solo per i valori inseriti a mano o per i referti che non lo riportano. Descrizione originale della tappa: Oggi `RANGE_RIF` copre **10 voci su 119**. Serve: (a) tabella di riferimento per tutte le voci, sesso-specifica dove serve, che Fabrizio valida voce per voce — e' contenuto clinico, non codice; (b) estrazione dei range STAMPATI dal laboratorio durante l'import (il campo `rf.range` esiste gia' vuoto), che hanno la precedenza perche' variano da laboratorio a laboratorio; (c) visualizzazione sempre visibile sotto ogni valore, accanto al pallino che c'e' gia'. **Opus · High · ON** — sono soglie cliniche.
...
### P122 — L'obiettivo del paziente: dal peso deciso a mano al traguardo derivato ✅ **CHIUSA E COLLAUDATA 26 lug 2026** (nata 25 lug 2026, 5ª sessione — tutte e 5 le tappe chiuse + 5 correzioni post-collaudo)

> 🔧 **Correzione del titolo (verificato 4 ago 2026).** Questo titolo diceva «Tappa 1 chiusa, tappe 2-5 aperte»: era **il testo più vecchio della voce**, rimasto fermo al momento in cui la scheda è nata. Contraddiceva il corpo della voce stessa — le tappe 2, 3, 4 e 5 sono tutte marcate «✅ CHIUSA» qui sotto, la riga **«P122 COMPLETA»** sta ~38 righe più in basso, e la SCHEDA in fondo dice **«CHIUSA E COLLAUDATA 26 lug 2026»** con il collaudo in studio su paziente reale. Confermato dalla Guida modello/effort in cima al file, dove P122 è già barrata come chiusa e collaudata il 26 lug 2026. Il titolo è stato allineato al resto; **nessun contenuto delle tappe è stato tolto**.
...
**✅ F5-salvaPaz chiusa nello stesso commit** *(rinominata da «F5» il 4 ago 2026: la sigla F5 era già usata per la vittoria gratuita PostgREST — v. nota di collisione nei findings trasversali)**:** `salvaPaz` preservava dal paziente esistente solo una **whitelist** di campi, ferma a prima delle ultime funzioni — salvare l'anagrafica cancellava in silenzio `p.percorso` (P115), `p.refertiSangue` (P118), `p.richiesteAnalisi` (P116), `p.consuntivo` e `p.creato`. Risolto nel punto di scrittura con `_pazPreservaCampi` (si riporta per costruzione tutto ciò che il form non produce), non allungando la lista. Dettaglio e lezione nel CHANGELOG.
...
**✅ TAPPA 3 CHIUSA 26 lug 2026 — modelli di periodizzazione + riallinea.** `_percorsoGeneraFasi` (pura): 4 modelli (ricomposizione / dimagrimento a cicli / massa / mantenimento-salute) con le regole cliniche codificate — deficit mai oltre 12 settimane, mantenimento 4 tra i blocchi, stabilizzazione 6 prima della massa, guardia a 8 cicli. Ritmo = TDEE × %fase ÷ 7700, lo stesso della proiezione P115. Le fasi generate sono fasi normali (modificabili una per una); la generazione chiede conferma prima di sostituire fasi esistenti; senza traguardo manda al pannello 🎯. **⏩ Riallinea a oggi**: il piano trasla con ancora sull'ultima pesata e si riprende esattamente dal giorno-fase dell'interruzione (fissato da test); il badge "fuori dalle fasi" è diventato "⏸ Il piano è rimasto indietro — capita in tutti i percorsi veri". Test 225 → 237 (`s2-percorso-generatore.test.js`).

**✅ CORREZIONI POST-COLLAUDO — 26 lug 2026.** Il primo collaudo sul campo (note in `NutriGest_P122_Collaudo_e_Correzioni.md`) ha trovato **due errori di modello, non di calcolo**:
1. **Il motore non sapeva cos'è la ricomposizione.** Conosceva solo massa magra costante o in calo: puntare al 9% di grasso veniva letto come "dimagrire 6.1 kg" anche su chi deve *perdere grasso e mettere muscolo*. Su 83 kg con 70 di magra, al 9%: magra ferma → 76.9 kg, +2 → 79.1, +4 → 81.3, +6 → **83.5** (peso invariato, obiettivo raggiunto). Ora esiste il modo `ricomposizione` (`peso = (magra + guadagno)/(1−T)`, default +2 kg), **prescelto dalla categoria dichiarata nella Tappa 2**, e il pannello mostra grasso/magra/peso invece di "da perdere X kg".
2. **Referti InBody incoerenti + quarta occorrenza del pattern "due fonti per lo stesso dato".** Un referto con peso 83, magra 70 e %grasso 10 produceva «massa grassa 13 kg (10%)»; e una sola riga dell'app derivava la FFM dalla percentuale mentre tutto il resto legge il campo `m` («FFM 74.7» in alto, «calcolato su FFM (70 kg)» in fondo alla stessa schermata). Ora la % è sempre derivata dalla massa magra usata, l'incoerenza viene **dichiarata con entrambi i traguardi possibili** (avviso, non blocco — scelta di Fabrizio), e la FFM ha una fonte sola. Non è cosmetico: al 9% il traguardo passa da 76.9 a 82.1 kg a seconda del dato usato.
3. Confronto aspettativa: riconosce le **direzioni opposte** (pesa 83, ne vuole 86, il traguardo lo porta a 79) guardando il peso attuale e lo scenario in uso, non il centro della fascia.
4. Etichette riscritte («quanto del calo è massa magra») e messaggio corretto per quota 0 (i due scenari *coincidono*, non "non calcolabile").
5. Traguardo scritto a mano nel campo: il disallineamento con quello clinico viene dichiarato, con il pulsante per registrarlo come scelta manuale.
Test 237 → **254** (`s2-traguardo-correzioni.test.js`).

**✅ TAPPA 4 CHIUSA 26 lug 2026 — traguardi multipli, condizione di uscita, chiusura con esito.** Promossa a necessaria dal collaudo: in ricomposizione il peso può non muoversi affatto mentre il percorso funziona.
- `p.obiettivoPercorso.traguardi[]`: sette tipi (% grasso, massa grassa, massa magra, peso, girovita, esame del sangue, **comportamento**), ognuno col valore attuale letto dai dati esistenti, partenza fotografata alla creazione, progresso 0-100% e verso dedotto dove serve. Dato mancante → "non disponibile", mai un numero inventato.
- **Comportamentali**: unico tipo senza misura automatica, si segnano ✓/✕ a mano. Libreria scelta da Fabrizio: passi · allenamenti · sonno · alcol · verdura · sgarri · acqua. Sono l'unica vittoria disponibile in un mese storto.
- **Condizione di uscita** della fase (% grasso / peso / massa magra): quando scatta l'app **suggerisce e basta** — scelta esplicita contro l'accorciamento automatico, fissata da un test che verifica che il piano non cambi di una virgola.
- **Chiusura fase con esito** 🔒: fotografa peso, % grasso e massa magra del giorno; riapribile.
- Bug corretto nello stesso giro: `_percorsoGet` ricostruiva la fase con i soli tre campi originari, quindi condizione/stato/esito sparivano per chi leggeva dalla normalizzazione (stessa famiglia di F5-salvaPaz).
...
### P61 — Validatore clinico post-generazione AI ✅ CHIUSA (commit `ed1e3e9`, 7 luglio 2026)
...
**SCHEDA:** Stato: ✅ **CHIUSA** (commit `ed1e3e9`, 7 lug 2026) · Priorità: era Alta (CRITICA) · C: 3 | I: 5 | R: 2 · Modello: Fable 5 (Ragionamento Attivo Alto) per la funzione + golden test; Sonnet (Media) per il pannello UI · Autonomia: **L0** sul motore, L1 sulla UI.
> 🔴 **CORREZIONE DI STATO (verificato 4 ago 2026).** Questa scheda ha detto «Stato: Da fare · Priorità: Alta (CRITICA)» per **quasi un mese dopo la chiusura**: è la divergenza più grave trovata dall'audit, perché una voce marcata CRITICA e da fare invita a rifare da zero un validatore clinico che è già in produzione. Verifica fatta al commit `c9fffac`: `grep -n "function validaPiano\|function validaGateExport" index.html` → **`validaPiano(piano, p)` e `validaGateExport(paziente, piano, onProceed)` esistono entrambe** in `index.html` (14 occorrenze complessive delle due sigle); esiste `test-suite/test/s2-valida-piano.test.js`; `grep -n "ed1e3e9" CHANGELOG.md` → *«P61 — Validatore clinico post-generazione (commit ed1e3e9), modello Fable 5»*, sessione pomeridiana del **7 luglio 2026**.
> **Il file si contraddiceva da solo:** più in alto, nei findings trasversali, la riga di riconciliazione dell'8 lug 2026 («**P78, P61, P62, P77 sono chiusi**») e il Blocco A («P78 ✅ → P61 ✅ …») davano P61 già chiusa. Fra scheda e riepilogo la regola del file dice che fa fede la scheda — e la scheda era quella sbagliata. Lezione: la regola «fa fede la scheda» protegge solo se la scheda viene aggiornata **nello stesso momento** in cui il commit entra nel CHANGELOG.
